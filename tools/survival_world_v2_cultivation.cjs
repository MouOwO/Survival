...
for(let q=0;q<4;q++){const p=originalIsland.worldPoint(4100,640,q);pavilion(...p,.8);}
for(const p of cloudArt.pavilions)for(let i=0;i<3;i++){
 const a=i*2.1+.5,x=p.x+Math.cos(a)*450,y=p.y+Math.sin(a)*450;
 if(nativePreserved(x,y)&&(originalIsland.round.islandDistance(x,y)>-200||originalIsland.round.paint([x,y])[0]<.45||originalIsland.round.stairHole(x,y)))continue;
