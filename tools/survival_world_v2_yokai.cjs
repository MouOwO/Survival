...
const nightArt={theme:'修仙云海',timeOfDay:.3,lanterns:[],wisps:[],removedMainIslandLanterns:[]};
let lanternId=0;
...
function nightLantern(x,y,scale=1){
 ...
 // Remove the main island's repeated torch posts, their flames and their local
 // lights together. Preserve the numbering/orientation of lamps in other rooms.
 const i=lanternId++;
 if(nativePreserved(x,y)){nightArt.removedMainIslandLanterns.push({x,y,id:i});return;}
 const z=surface(x,y);
 ...
 for(const time of ['day','night'])Object.assign(light,{['color_'+time]:'244 198 139 255',['light_scale_'+time]:'2.4',['ambient_color_'+time]:'184 165 147 255',['ambient_scale_'+time]:'1.05',['fog_start_'+time]:'20000',['fog_end_'+time]:'40000',['fog_height_'+time]:'0',['fow_darkess_'+time]:'1',['light_direction_'+time]:'66 330 0'});
...
function ghostLight(x,y){return;const z=surface(x,y)+65;entity('info_particle_system','yokai_wisp_'+nightArt.wisps.length,[x,y,z],{effect_name:'particles/survival_world_v2/yokai_wisps.vpcf',start_active:'1'});nightArt.wisps.push({x,y,z});}
for(let q=0;q<4;q++){
