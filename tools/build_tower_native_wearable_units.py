"""Synchronize native-wearable tower carrier units from authoritative CSVs."""
...
def sync_csv_sources(check: bool) -> bool:
    ...
    def update_catalog(values, row, headers):
        ...
        projections = {
            ...
            "material_group": "",
            "notes": stage["notes"]
                + "；基础穿戴由代理单位Creature.AttachWearables按已验证ItemDef挂载。",
...
def render_block(
    ...
    item_rows = [row for row in wearables if row.get("item_def", "").strip()]
    for index, row in enumerate(item_rows, 1):
        lines.extend([
            f'                "{index}"',
            "                {",
            f'                    "ItemDef" "{row["item_def"].strip()}"',
            "                }",
        ])
    lines.extend(["            }", "        }", "    }"])
