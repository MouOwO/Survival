...
grid=grid.replace(/^(\t{6})"([^"]+)" "(\w+)_array"\s*\[([^\]]*)\]/gm,(all,indent,k,t,body)=>{
 ...
 for(let i=0;i<count;i++){
  ...
  if(k==='cellsHidden')a[i]=nativePreserved(x,y)?1:at(x,y)?.material===M.water?0:1;
