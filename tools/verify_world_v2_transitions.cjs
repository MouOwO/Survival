...
assert.equal(islands.length,4);assert(lake.shallow);assert(lake.polygon.length>32);const [cx,cy]=surface.crossCenter,ib=bounds(islands.flatMap(a=>a.polygon)),lb=bounds(lake.polygon);
assert(Math.abs(lb[2]-lb[0]-4608)<.01);assert.equal(surface.crossWidth,1152);
...
for(let i=0;i<8;i++){const a=(i+.5)*Math.PI/4;tests.push({name:'round_lake_'+i,x:cx+928*Math.cos(a),y:cy+928*Math.sin(a),walk:true,shallow:true});}
