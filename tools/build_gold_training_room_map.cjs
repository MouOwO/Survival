...
const root=path.resolve(__dirname,'..'),out=path.join(root,'output/gold_training_room');
...
const prefix=source.slice(0,source.indexOf("water('central_sea'"))
 .replace('../output/zombie_island_v1','../output/gold_training_room').replace('zombie-abyss-v1-','gold-training-room-');
...
const supportMaterial='materials/gold_training_room/floor_support.vmat';
const materialDir=path.join(out,'source/materials/gold_training_room');
fs.writeFileSync(path.join(materialDir,'floor_support.vmat'),fs.readFileSync(path.join(materialDir,'mortar.vmat'),'utf8').replace(/}\s*$/,'"Attributes" { "dota.nav.walkable" "1" }\n}\n'));
...
for(let i=0;i<layout.placements.length;i++){
 ...
 entity('prop_static','gold_room_'+String(i).padStart(4,'0')+'_'+path.basename(p.name,'.vmdl'),
  [p.origin[0],p.origin[1],zbase+p.origin[2]],
  {model:p.native?p.name:'models/gold_training_room/'+p.name+'.vmdl',solid:a?.collision_hulls?'6':'0',rendercolor:'255 255 255',disableshadows:'0'},
...
box(0,0,4096,4096,zbase-29,zbase-120,'materials/gold_training_room/earth.vmat');
entity('world_bounds','gold_room_review_bounds',[0,0,0],{min:'-2048 -2048 0',max:'2048 2048 0'});
entity('info_player_start_goodguys','gold_room_review_start',[0,700,152]);
entity('info_player_start','gold_room_editor_start',[0,700,152]);
entity('info_player_start_badguys','gold_room_enemy_start',[0,-140,152]);
entity('ent_dota_game_events','gold_room_events',[0,0,0]);
entity('env_global_light','gold_room_daylight',[0,0,2400],{color:'255 243 218 255',lightscale:'1.8',ambientcolor1:'198 208 215 255',ambientscale1:'1.15',ambientcolor2:'171 184 186 255',ambientscale2:'.8',ambientcolor3:'142 145 129 255',groundscale:'.65',enableshadows:'1',StartDisabled:'0'},'55 315 0');
entity('env_tonemap_controller','gold_room_tonemap',[0,0,0],{UseCustomAutoExposureMin:'1',UseCustomAutoExposureMax:'1',AutoExposureMin:'.9',AutoExposureMax:'1.1'});
...
save('gold_training_room_review',[grid,...children]);
save('prefabs/gold_training_room',prefabChildren);
fs.writeFileSync(path.join(out,'map_manifest.json'),JSON.stringify({review:'gold_training_room_review',prefab:'prefabs/gold_training_room',instances:layout.placements.length,customAssets:assets.length,nativeAssets:layout.native_models,markers:layout.markers.map(m=>m.name),floorZ:zbase,openGridCells:openCells,emissiveEntities:0,mainMapModified:false},null,2));
console.log('GOLD_ROOM_MAP',layout.placements.length,'instances',openCells,'walkable cells');
