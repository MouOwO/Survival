...
function height(x,y){const p=locate(x,y);if(!p)return null;if(shallow(x,y))return 3;if(!p.valid)return 3;return Number(scalar('verticesHeight',p.x,p.y,256))+3;}
...
function sample(key,x,y,original,edgeVertical=false){
 ...
 if(key==='gridnavFlags')return isShallow||land?'0':'1';
...
module.exports={center,pivot,locate,shallow,height,sample,transform,arrays,originalLand};
