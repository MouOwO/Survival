...
function shallow(x,y){const dx=Math.abs(x-center[0]),dy=Math.abs(y-center[1]),r=Math.max(dx,dy);return r<=1792+outward&&Math.min(dx,dy)<=channelHalfWidth(r);}
...
const levels={water:416,bed:396,attack:640,tower:768,stairStart:1792,stairEnd:2304,stairWidth:576,outward,lateral,crossHalfLength:1792+outward,crossHalfWidth:320+lateral,taperStart:1152,taperEnd:2048,gateWaterWidth:640};
...
module.exports={center,pivot,locate,shallow,height,sample,transform,arrays,originalLand,levels,approach,replacedStairTile,preserved,worldPoint,innerBankTile,channelHalfWidth,replacedTowerStairTile};
