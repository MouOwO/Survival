...
for(const [cls,props]of Object.entries(lighting)){for(const k of Object.keys(props))if(k.endsWith('map_texture')||k==='height_fog_textureopacity')delete props[k];if(cls==='ent_dota_lightinfo'){for(const time of ['day','night'])Object.assign(props,{['color_'+time]:'239 237 222 255',['light_scale_'+time]:'2.4',['ambient_color_'+time]:'185 205 218 255',['ambient_scale_'+time]:'1.25',['fog_start_'+time]:'20000',['fog_end_'+time]:'40000',['fog_height_'+time]:'0',['fow_darkess_'+time]:'1',['light_direction_'+time]:'66 330 0'});props.farz_override='45000';}else Object.assign(props,{color:'239 237 222 255',lightscale:'2.4',ambientcolor1:'185 205 218 255',ambientscale1:'1.25',ambientcolor2:'151 179 190 255',ambientscale2:'.35',ambientcolor3:'151 179 190 255',groundscale:'.38',fow_darkness:'1'});entity(cls,NAME+'_'+cls,[0,0,2500],props,'66 330 0');}
entity('env_tonemap_controller','daylight_exposure',[0,0,0],{minexposure:'1.0',maxexposure:'1.2',master:'1',rate:'2'});
