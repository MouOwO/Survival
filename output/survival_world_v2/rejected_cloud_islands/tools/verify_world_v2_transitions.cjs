...
assert.equal(surface.architecture,'separate supported islands and retained water basins');
const before=JSON.parse(fs.readFileSync(path.join(out,'before_separate_cloud_islands/surface_design.json')));
const triangles=surface.meshAudit.reduce((n,c)=>n+c.topTriangles,0);
assert(triangles<before.meshAudit.reduce((n,c)=>n+c.topTriangles,0)*.8,'Unused mountain terrain must actually be removed');
assert(surface.removedMountainCells>12000);
assert(surface.canopyCount<150,'Old continuous mountain forest must be removed');
const support=JSON.parse(fs.readFileSync(path.join(out,'island_support_audit.json')));
assert(support.markers.every(m=>m.supported),'Every gameplay marker needs a supported platform');
