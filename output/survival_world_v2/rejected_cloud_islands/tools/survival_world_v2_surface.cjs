...
// Separate supported platforms. Unused mountain terrain is physically absent.
const ornamentalIslands=[[-11800,15100,900,800,780],[5200,-13500,950,1000,780],[13700,-7100,850,850,652],[0,-14000,1850,1350,740]];
function ornamentalSupport(x,y){return ornamentalIslands.find(([cx,cy,rx,ry])=>Math.hypot((x-cx)/rx,(y-cy)/ry)<=1);}
function groundSupport(x,y){
 if(nativePreserved(x,y)||at(x,y)||ornamentalSupport(x,y))return true;
 return nearbyPaint(x,y).some(a=>a.name!=='paving'&&edgeDistance(x,y,a.polygon)<=96);
}
function rawSurface(x,y){const a=at(x,y);if(a)return a.material===M.water&&!a.shallow?254:Math.round(a.z/128)*128+12;
 const ornament=ornamentalSupport(x,y);if(ornament)return ornament[4];
 let nearest=null,d=Infinity;for(const a of nearbyPaint(x,y)){if(a.name==='paving')continue;const v=edgeDistance(x,y,a.polygon);if(v<d){d=v;nearest=a;}}
 return nearest&&d<512?Math.round(nearest.z/128)*128+12:-768;
}
...
function terrainHeight(x,y){let h=rawSurface(x,y);
...
for(const a of terrainPaint){if(a.nativeReuse||a.material===M.water)continue;for(let y=Math.floor((a.bb[1]-640)/512);y<=Math.floor((a.bb[3]+640)/512);y++)for(let x=Math.floor((a.bb[0]-640)/512);x<=Math.floor((a.bb[2]+640)/512);x++){const key=x+','+y;if(!paintBins.has(key))paintBins.set(key,[]);paintBins.get(key).push(a);}}
...
function continuousPaint(v,m){if(m>=paletteNames.length||m===8)return [0,0,0,0];const [x,y]=v,key=x+','+y+','+m;if(paintCache.has(key))return paintCache.get(key);
 if(paletteNames[m]==='martial_forest')return [0,0,1,0];
 ...
 for(const a of shapes){const own=theme(a),d=edgeDistance(x,y,a.polygon),within=inside(x,y,a.polygon),signed=within?d:-d;
  ...
  if(a.combatArt)seam=Math.min(seam,smooth((d-128)/200));
  ...
  const alpha=smooth((signed+280+(n-.5)*260)/760);if(!alpha)continue;let target;
  if(t==='snow')target=[1,0,0];else if(t==='thaw')target=[1,0,0];else if(t==='volcanic')target=martial?[1,.35+.25*n,0]:[.58+.30*(1-n),1,0];
  else if(a.material===M.dirt)target=[0,1,0];else if(a.material===M.paving||a.material===M.stone||a.material===M.corrupt)target=[0,0,1];else target=martial?[0,0,1]:[1,0,0];
...
for(let cy=0;cy<256;cy+=chunkCells)for(let cx=0;cx<256;cx+=chunkCells){const verts=[],faces=[],ids=new Map(),boundary=new Map();
 ...
 for(let gy=cy;gy<cy+chunkCells;gy++)for(let gx=cx;gx<cx+chunkCells;gx++){const x=-16384+(gx+.5)*gridStep,y=-16384+(gy+.5)*gridStep;if(nativePreserved(x,y)||!groundSupport(x,y))continue;const a=vertex(gx,gy),b=vertex(gx+1,gy),c=vertex(gx+1,gy+1),d=vertex(gx,gy+1);const slope=Math.hypot(terrainHeight(x+64,y)-terrainHeight(x-64,y),terrainHeight(x,y+64)-terrainHeight(x,y-64))/128,m=paletteNames.indexOf(localTheme(x,y));triangle([a,b,c],m);triangle([a,c,d],m);}
 if(!faces.length)continue;const topFaces=faces.length,bottoms=new Map(),edges=[...boundary.values()];
 function bottom(id){if(!bottoms.has(id)){bottoms.set(id,verts.length);verts.push([verts[id][0],verts[id][1],verts[id][2]-220]);}return bottoms.get(id);}
 // Mirror each top triangle. A fan to one chunk center self-intersects when
 // islands split a chunk into disconnected pieces or leave concave gaps.
 for(let i=0;i<topFaces;i++)faces.push({v:faces[i].v.map(bottom).reverse(),m:paletteNames.length});
 for(const [a,b] of edges){const ba=bottom(a),bb=bottom(b);faces.push({v:[b,a,ba,bb],m:8});}
...
// Native cliff tiles omit unsupported high shore combinations. A closed rock
// foundation below their authored top surfaces fills the raised island's skirt.
function islandFoundationHeight(x,y){
 if(originalIsland.shallow(x,y))return 372;
 const gx=Math.floor((x+16384)/256),gy=Math.floor((y+16384)/256),u=(x+16384)/256-gx,v=(y+16384)/256-gy;
 const h=(ix,iy)=>{const wx=-16384+ix*256,wy=-16384+iy*256,p=originalIsland.locate(wx,wy);return originalIsland.shallow(wx,wy)?372:originalIsland.originalLand(p)?Math.min(636,originalIsland.height(wx,wy)*128-28):254;};
 return (h(gx,gy)*(1-u)+h(gx+1,gy)*u)*(1-v)+(h(gx,gy+1)*(1-u)+h(gx+1,gy+1)*u)*v;
}
for(let oy=preserved[1];oy<preserved[3];oy+=1280)for(let ox=preserved[0];ox<preserved[2];ox+=1280){
 const nx=Math.min(10,(preserved[2]-ox)/128),ny=Math.min(10,(preserved[3]-oy)/128),verts=[],faces=[];
 for(let y=0;y<=ny;y++)for(let x=0;x<=nx;x++)verts.push([ox+x*128,oy+y*128,islandFoundationHeight(ox+x*128,oy+y*128)]);
 const count=verts.length;for(let i=0;i<count;i++)verts.push([verts[i][0],verts[i][1],-512]);
 const at=(x,y)=>y*(nx+1)+x;
 for(let y=0;y<ny;y++)for(let x=0;x<nx;x++){const a=at(x,y),b=at(x+1,y),c=at(x+1,y+1),d=at(x,y+1);faces.push({v:[a,b,c],m:0},{v:[a,c,d],m:0},{v:[d+count,c+count,b+count,a+count],m:1});}
 const skirt=(a,b)=>faces.push({v:[b,a,a+count,b+count],m:0});
 for(let x=0;x<nx;x++){skirt(at(x,0),at(x+1,0));skirt(at(x+1,ny),at(x,ny));}
 for(let y=0;y<ny;y++){skirt(at(nx,y),at(nx,y+1));skirt(at(0,y+1),at(0,y));}
 mesh(verts,faces,['materials/survival_world_v2/transition_forest.vmat','materials/tools/toolsnodraw.vmat'],'255 255 255 255');
}
for(let q=0;q<4;q++){
 ...
 const attack=originalIsland.transform(512,128,q),towerLeft=originalIsland.transform(-1792,768,q),towerRight=originalIsland.transform(-1792,-512,q);
...
let canopyCount=0;
for(let y=-15800;y<16000;y+=270)for(let x=-15800;x<16000;x+=270){const px=x+(rnd()-.5)*150,py=y+(rnd()-.5)*150;if(!ornamentalSupport(px,py)||springRadius(px,py)<2.2||nativePreserved(px,py)||at(px,py)||at(px+320,py)||at(px-320,py)||at(px,py+320)||at(px,py-320))continue;if(variation(px*.48,py*.48)<.38||rnd()<.12)continue;
...
fs.writeFileSync(path.join(OUT,'surface_design.json'),JSON.stringify({mainIsland:'four rotated original U islands',sourceMap:'template_map.vmap',crossCenter:originalIsland.center,crossLength:originalIsland.levels.crossHalfLength*2,crossWidth:originalIsland.levels.crossHalfWidth*2,groundSurfaceZ:396,waterZ:416,transitionWidth:760,architecture:'separate supported islands and retained water basins',removedMountainCells:256*256-(preserved[2]-preserved[0])*(preserved[3]-preserved[1])/128**2-meshAudit.reduce((n,c)=>n+c.topTriangles/2,0),ornamentalIslands,preserved,gridStep,meshAudit,canopyCount,groundRoots:groundRoots.map(a=>({name:a.name,theme:theme(a),z:level(a)+12})),lilySource:'Native tileset entities disabled'},null,2));
