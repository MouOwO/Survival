...
function platform(name,p,z=384,material=M.grass,room=false){const combatArt=room||/^(rebirth_|flame_troll|ice_elegy|synthesis_gem|commandment_|polar_crystal_mobs|molten_core_mobs|seven_sins|original_challenge_reserve)/.test(name);const a={name,polygon:p,z,material,room,combatArt};land.push(a);terrainPaint.push(a);if(room)rooms.push(name);}
...
// INSERT SURFACES
// INSERT SCENERY
// INSERT MARTIAL ROOMS
// INSERT CULTIVATION
// INSERT NIGHT ART
for(let i=0;i<300;i++){const x=-15400+rnd()*30800,y=-15000+rnd()*30000;if(!ornamentalSupport(x,y)||springRadius(x,y)<2.2||at(x,y)||at(x+220,y)||at(x-220,y)||at(x,y+220)||at(x,y-220))continue;entity('prop_static','ridge_rock_'+i,[x,y,surface(x,y)],{model:'models/props_nature/river_rocks00'+(1+i%3)+'.vmdl',solid:'0',rendercolor:'106 118 119',renderamt:'255'},`0 ${rnd()*360} 0`,2+rnd()*3);}
const lighting=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../output/reference_asset_study/lighting.json'),'utf8'));
for(const [cls,props]of Object.entries(lighting)){for(const k of Object.keys(props))if(k.endsWith('map_texture')||k==='height_fog_textureopacity')delete props[k];if(cls==='ent_dota_lightinfo'){for(const time of ['day','night'])Object.assign(props,{['color_'+time]:'241 239 231 255',['light_scale_'+time]:'1.9',['ambient_color_'+time]:'209 222 229 255',['ambient_scale_'+time]:'1.7',['fog_start_'+time]:'20000',['fog_end_'+time]:'40000',['fog_height_'+time]:'0',['fow_darkess_'+time]:'1',['light_direction_'+time]:'66 330 0'});props.farz_override='45000';}else Object.assign(props,{color:'241 239 231 255',lightscale:'1.9',ambientcolor1:'209 222 229 255',ambientscale1:'1.7',ambientcolor2:'183 201 210 255',ambientscale2:'.6',ambientcolor3:'183 201 210 255',groundscale:'.58',fow_darkness:'1'});entity(cls,NAME+'_'+cls,[0,0,2500],props,'66 330 0');}
entity('env_tonemap_controller','daylight_exposure',[0,0,0],{minexposure:'1.05',maxexposure:'1.05',master:'1',rate:'2'});
...
grid=grid.replace(/^(\t{6})"([^"]+)" "(\w+)_array"\s*\[([^\]]*)\]/gm,(all,indent,k,t,body)=>{
 ...
 for(let i=0;i<count;i++){
  ...
  // This one native cliff tile per island is replaced by the entrance stair
  // and landing meshes, including navigation. Hiding avoids a second collision
  // surface underneath the walkable cap influencing the nav rasterization.
  if(k==='cellsHidden'&&(originalIsland.replacedStairTile(x,y)||originalIsland.innerBankTile(x,y)||originalIsland.replacedTowerStairTile(x,y)))a[i]=1;
...
// Remove old ridge props whose supporting terrain was deliberately cut away.
let removedUnsupportedProps=0;
for(let i=children.length-1;i>=0;i--){const s=children[i];if(!s.includes('"prop_static"')&&!s.includes('"ent_dota_tree"'))continue;const p=s.match(/"origin" "vector3" "([^"]+)"/);if(!p)continue;const [x,y]=p[1].split(' ').map(Number);if(!groundSupport(x,y)){children.splice(i,1);removedUnsupportedProps++;}}
fs.writeFileSync(path.join(OUT,'island_support_audit.json'),JSON.stringify({removedUnsupportedProps,markers:markers.map(m=>({name:m.name,supported:groundSupport(m.x,m.y)}))},null,2));
// Align scenery and markers to the native grid surface, retaining their authored scale.
for(let i=0;i<children.length;i++)if(!children[i].includes('cultivation_')&&!children[i].includes('island_bank_')&&(children[i].includes('"prop_static"')||children[i].includes('"ent_dota_tree"')||children[i].includes('"info_target"')))children[i]=children[i].replace(/"origin" "vector3" "([^\"]+)"/,(m,p)=>{const v=p.split(' ').map(Number);v[2]=surface(v[0],v[1]);return '"origin" "vector3" "'+v.join(' ')+'"';});
