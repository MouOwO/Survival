...
function terrainHeight(x,y){let h=0;for(let j=-1;j<=1;j++)for(let i=-1;i<=1;i++)h+=rawSurface(x+i*128,y+j*128)*(i===0?2:1)*(j===0?2:1);return h/16;}
...
function continuousPaint(v,m){if(m>=paletteNames.length)return [0,0,0,0];const [x,y]=v,key=x+','+y+','+m;if(paintCache.has(key))return paintCache.get(key);
 const t=paletteNames[m],n=variation(x,y),shapes=nearbyPaint(x,y);let w=[.18+.45*n,0,0],seam=1;
 for(const a of shapes){const own=theme(a),d=edgeDistance(x,y,a.polygon),within=inside(x,y,a.polygon),signed=within?d:-d;
  if(a.name!=='paving'&&own!==t){seam=Math.min(seam,smooth((d-24)/260));continue;}
  const alpha=smooth((signed+160+(n-.5)*210)/440);if(!alpha)continue;let target;
  if(t==='snow')target=[1,0,0];else if(t==='thaw')target=[1,0,0];else if(t==='volcanic')target=[.65+.20*(1-n),1,0];else if(t==='corrupt')target=[.10+.12*n,.70,0];
  else if(a.material===M.dirt)target=[0,1,0];else if(a.material===M.paving||a.material===M.stone)target=[0,0,1];else target=[1,0,0];
 ...
 if(t!=='forest'){let edge=0;for(const a of shapes)if(theme(a)===t&&inside(x,y,a.polygon))edge=Math.max(edge,smooth((edgeDistance(x,y,a.polygon)-12)/260));seam*=edge;}
 if(t==='forest'||t==='thaw'||t==='corrupt')w[0]*=.36;
...
let canopyCount=0;
for(let y=-15800;y<16000;y+=270)for(let x=-15800;x<16000;x+=270){const px=x+(rnd()-.5)*150,py=y+(rnd()-.5)*150;if(nativePreserved(px,py)||at(px,py)||at(px+320,py)||at(px-320,py)||at(px,py+320)||at(px,py-320))continue;if(variation(px*.48,py*.48)<.38||rnd()<.12)continue;
 entity('ent_dota_tree','ridge_canopy_'+canopyCount++,[px,py,surface(px,py)],{model:canopyCount%5===0?'models/props_tree/tree_oak_spring_01.vmdl':'models/props_tree/tree_pine_01.vmdl',solid:'0',body:'1',rendercolor:'163 186 163 255',renderamt:'255'},`0 ${rnd()*360} 0`,1.35+rnd()*.75);}
