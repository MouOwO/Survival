...
for(const a of land){
 if(a.nativeReuse||!(a.room||a.name.includes('camp')||a.name.includes('commandment')||a.name.includes('seven')))continue;
 const p=a.polygon,c=p.reduce((s,v)=>[s[0]+v[0]/p.length,s[1]+v[1]/p.length],[0,0]);
 const v=p[0],x=c[0]+(v[0]-c[0])*.77,y=c[1]+(v[1]-c[1])*.77;nightLantern(x,y,.9);
 if(a.name.includes('commandment')||a.name.includes('seven'))ghostLight(x+160,y-160);
}
