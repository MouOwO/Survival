...
for(const f of islandLevels.flights){
 ...
 lua+=`do local a=${vec(f.foot)};local b=${vec(f.landing)};local ok=GridNav:CanFindPath(a,b);local prev=0;local heights={};for i=0,24 do local p=a+(b-a)*(i/24);local h=GetGroundHeight(p,nil);table.insert(heights,math.floor(h));if not GridNav:IsTraversable(p) or GridNav:IsBlocked(p) or (i>0 and (h<prev-4 or h-prev>64)) then ok=false;print('[WORLD_V2_STAIR_DETAIL]',${f.q},i,p.x,p.y,h,prev,GridNav:IsTraversable(p),GridNav:IsBlocked(p)) end;prev=h end;print('[WORLD_V2_ISLAND_STAIRS]',${f.q},table.concat(heights,','),ok and 'PASS' or 'FAIL') end\n`;
}
