...
assert.equal(islands.length,4);assert(lake.shallow);assert.equal(lake.polygon.length,12);const [cx,cy]=surface.crossCenter,ib=bounds(islands.flatMap(a=>a.polygon)),lb=bounds(lake.polygon);
...
for(const f of islandLevels.flights){
 ...
 lua+=`do local a=${vec(f.foot)};local b=${vec(f.landing)};local ok=GridNav:CanFindPath(a,b);local prev=0;local heights={};for i=0,24 do local p=a+(b-a)*(i/24);for _,offset in ipairs({-384,384}) do local side=p+Vector(-(b.y-a.y),b.x-a.x,0):Normalized()*offset;if not GridNav:IsTraversable(side) or GridNav:IsBlocked(side) then ok=false;print('[WORLD_V2_STAIR_EDGE]',${f.q},i,offset) end end;local h=GetGroundHeight(p,nil);table.insert(heights,math.floor(h));if not GridNav:IsTraversable(p) or GridNav:IsBlocked(p) or (i>0 and (h<prev-4 or h-prev>64)) then ok=false;print('[WORLD_V2_STAIR_DETAIL]',${f.q},i,p.x,p.y,h,prev,GridNav:IsTraversable(p),GridNav:IsBlocked(p)) end;prev=h end;print('[WORLD_V2_ISLAND_STAIRS]',${f.q},table.concat(heights,','),ok and 'PASS' or 'FAIL') end\n`;
}
for(let q=0;q<4;q++){const c=Math.cos(q*Math.PI/2),s=Math.sin(q*Math.PI/2);lua+=`do local ok=true;local count=0;for r=128,2176,128 do for _,v in ipairs({-448,0,448}) do local p=Vector(${cx}+r*${Math.round(c)}-v*${Math.round(s)},${cy}+r*${Math.round(s)}+v*${Math.round(c)},500);local h=GetGroundHeight(p,nil);count=count+1;if not GridNav:IsTraversable(p) or GridNav:IsBlocked(p) or math.abs(h-396)>4 then ok=false;print('[WORLD_V2_CHANNEL_DETAIL]',${q},r,v,h,GridNav:IsTraversable(p),GridNav:IsBlocked(p)) end end end;print('[WORLD_V2_CHANNEL_WIDTH]',${q},count,ok and 'PASS' or 'FAIL') end\n`;}
