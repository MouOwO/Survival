...
function shallow(x,y){const dx=Math.abs(x-center[0]),dy=Math.abs(y-center[1]);return Math.max(dx,dy)<=1792+outward&&Math.min(dx,dy)<=320+lateral;}
...
const levels={water:416,bed:396,attack:640,tower:768,stairStart:1792,stairEnd:2304,stairWidth:1088,outward,lateral,crossHalfLength:1792+outward,crossHalfWidth:320+lateral};
...
function innerBankTile(x,y){const dx=Math.abs(x-center[0]),dy=Math.abs(y-center[1]),r=Math.max(dx,dy),v=Math.min(dx,dy);return r>=512&&r<=2304&&v>=512&&v<=768;}
...
module.exports={center,pivot,locate,shallow,height,sample,transform,arrays,originalLand,levels,approach,replacedStairTile,preserved,worldPoint,innerBankTile};
