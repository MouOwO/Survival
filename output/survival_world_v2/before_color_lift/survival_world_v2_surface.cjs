...
const paletteNames=['forest','snow','thaw','volcanic','corrupt','martial_forest','martial_snow','martial_volcanic'],paintCache=new Map();
function continuousPaint(v,m){if(m>=paletteNames.length)return [0,0,0,0];const [x,y]=v,key=x+','+y+','+m;if(paintCache.has(key))return paintCache.get(key);
 ...
 for(const a of shapes){const own=theme(a),d=edgeDistance(x,y,a.polygon),within=inside(x,y,a.polygon),signed=within?d:-d;
  ...
  else if(a.material===M.dirt)target=[0,1,0];else if(a.material===M.paving||a.material===M.stone||a.material===M.corrupt)target=[0,0,1];else target=martial?[.25,.66,0]:[1,0,0];
...
for(let q=0;q<4;q++){
 ...
 for(let i=0;i<n;i++){
  ...
  prism(p,z,360,'materials/survival_world_v2/island_stairs.vmat','materials/survival_world_v2/island_stairs.vmat','255 255 255 255');
 }
 ...
 prism(landingPad,islandLevels.attack+2,360,'materials/survival_world_v2/island_stairs.vmat','materials/survival_world_v2/island_stairs.vmat','255 255 255 255');
...
fs.writeFileSync(path.join(OUT,'surface_design.json'),JSON.stringify({mainIsland:'four rotated original U islands',sourceMap:'template_map.vmap',crossCenter:originalIsland.center,crossLength:originalIsland.levels.crossHalfLength*2,crossWidth:originalIsland.levels.crossHalfWidth*2,groundSurfaceZ:396,waterZ:416,transitionWidth:440,architecture:'continuous closed heightfield',preserved,gridStep,meshAudit,canopyCount,groundRoots:groundRoots.map(a=>({name:a.name,theme:theme(a),z:level(a)+12})),lilySource:'Native tileset entities disabled'},null,2));
