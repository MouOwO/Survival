...
for(let q=0;q<4;q++)for(const side of [-1,1]){
 ...
 const stairMaterial='materials/survival_world_v2/island_stairs.vmat';
 const slab=(a,b,lo,hi,z)=>{const p=[[a,lo],[b,lo],[b,hi],[a,hi]].map(v=>pt(...v));if(side<0)p.reverse();prism(p,z,620,stairMaterial,stairMaterial,'255 255 255 255');};
