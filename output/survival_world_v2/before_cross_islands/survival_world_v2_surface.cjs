...
const groundRoots=land.filter((a,i)=>!land.slice(0,i).some(b=>level(b)===level(a)&&a.polygon.every(v=>inside(v[0],v[1],b.polygon))));
function groundPaint(a,x,y){
 ...
 if(t==='volcanic'){const inner=smooth((d-180)/300);return [0,.24+.15*n,inner*.3*n,inner*.30*smooth((n-.40)*2)];}
 if(t==='corrupt')return [0,1-smooth((d-30)/360),.20+.38*n,0];
 if(t==='snow')return [0,.80+.20*n,.10*n,0];
...
if(shallow){prism(shallow.polygon,416,414,'materials/survival_world_v2/shallow_water.vmat','materials/tools/toolsnodraw.vmat','255 255 255 255');}
fs.writeFileSync(path.join(OUT,'surface_design.json'),JSON.stringify({roundIslandRadius:4608,squareLakeSide:2048,groundSurfaceZ:396,waterZ:416,transitionWidth:280,groundRoots:groundRoots.map(a=>({name:a.name,theme:theme(a),z:level(a)+12})),lilySource:'Native tileset entities; disabled in addon-local templates'},null,2));
