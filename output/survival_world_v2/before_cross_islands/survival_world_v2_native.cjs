...
function heightAt(x,y){const a=at(x,y);if(a)return a.material===M.water&&!a.shallow?2:Math.max(1,Math.ceil(a.z/128));const noise=(Math.sin(x/1100+y/1600)+Math.cos(y/1000-x/1400)+2)/4;return 5+Math.floor(noise*5);}
...
for(const a of land){if(a.name.includes('camp')||a.room||a.name.includes('rebirth')||a.name.includes('commandment'))continue;for(let i=0;i<130;i++){const p=a.polygon,j=Math.floor(rnd()*p.length),u=rnd(),v=p[j],b=p[(j+1)%p.length],cx=p.reduce((s,q)=>s+q[0],0)/p.length,cy=p.reduce((s,q)=>s+q[1],0)/p.length,t=.80+rnd()*.15;const x=cx+(v[0]+u*(b[0]-v[0])-cx)*t,y=cy+(v[1]+u*(b[1]-v[1])-cy)*t;const current=at(x,y);if(!current||current.material===M.water||current.material===M.lava||current.material===M.snow)continue;entity('prop_static','margin_detail_'+node,[x,y,surface(x,y)],{model:theme(a)==='forest'?details[i%4]:'models/props_nature/river_rocks00'+(1+i%3)+'.vmdl',solid:'0',rendercolor:theme(a)==='snow'?'193 212 220':theme(a)==='volcanic'?'114 102 91':'155 161 139',renderamt:'255',skin:theme(a)==='forest'&&i%4===1?'3':'default'},`0 ${rnd()*360} 0`,.7+rnd()*.6);}}
// INSERT SURFACES
...
for(let i=0;i<4;i++)entity('info_target','player_'+i+'_builder_spawn',[X(154+i*3),Y(79)+3200,550]);
