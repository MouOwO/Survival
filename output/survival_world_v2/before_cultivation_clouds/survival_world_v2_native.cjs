...
for(const [cls,props]of Object.entries(lighting)){for(const k of Object.keys(props))if(k.endsWith('map_texture')||k==='height_fog_textureopacity')delete props[k];if(cls==='ent_dota_lightinfo'){for(const time of ['day','night'])Object.assign(props,{['color_'+time]:'185 202 235 255',['light_scale_'+time]:'2.2',['ambient_color_'+time]:'148 169 198 255',['ambient_scale_'+time]:'1.0',['fog_start_'+time]:'20000',['fog_end_'+time]:'40000',['fog_height_'+time]:'0',['fow_darkess_'+time]:'1',['light_direction_'+time]:'66 330 0'});props.farz_override='45000';}else Object.assign(props,{color:'185 202 235 255',lightscale:'2.2',ambientcolor1:'148 169 198 255',ambientscale1:'1.0',ambientcolor2:'110 139 170 255',ambientscale2:'.35',ambientcolor3:'110 139 170 255',groundscale:'.38',fow_darkness:'1'});entity(cls,NAME+'_'+cls,[0,0,2500],props,'66 330 0');}
entity('env_tonemap_controller','daylight_exposure',[0,0,0],{minexposure:'.90',maxexposure:'1.15',master:'1',rate:'2'});
...
for(let i=0;i<children.length;i++)if(!children[i].includes('island_bank_')&&(children[i].includes('"prop_static"')||children[i].includes('"ent_dota_tree"')||children[i].includes('"info_target"')))children[i]=children[i].replace(/"origin" "vector3" "([^\"]+)"/,(m,p)=>{const v=p.split(' ').map(Number);v[2]=surface(v[0],v[1]);return '"origin" "vector3" "'+v.join(' ')+'"';});
