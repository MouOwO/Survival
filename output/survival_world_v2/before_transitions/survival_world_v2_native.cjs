...
function room(name,x,y,w=13,h=10){
 ...
 const models={forest:'models/props_tree/tree_oak_spring_01.vmdl',autumn:'models/props_tree/tree_oak_01b.vmdl',snow:'models/props_tree/tree_pine_01_heavysnow.vmdl',dire:'models/props_tree/dire_tree007.vmdl'};
...
for(const a of land.filter(a=>a.name.startsWith('commandment_')||a.room)){
...
function heightAt(x,y){const a=at(x,y);if(a)return a.material===M.water?2:Math.max(1,Math.ceil(a.z/128));const noise=(Math.sin(x/1100+y/1600)+Math.cos(y/1000-x/1400)+2)/4;return 5+Math.floor(noise*5);}
...
function surface(x,y){const ix=Math.max(0,Math.min(127,Math.floor((x+16384)/256))),iy=Math.max(0,Math.min(127,Math.floor((y+16384)/256)));return Math.max(heights[iy*129+ix],heights[iy*129+ix+1],heights[(iy+1)*129+ix],heights[(iy+1)*129+ix+1])*128;}
function biomeAt(a){if(!a)return 1;if(a.biome==='autumn')return 3;if(a.material===M.snow)return 2;if(a.material===M.lava||a.material===M.corrupt)return 1;return 0;}
...
for(const a of land){if(a.name.includes('camp')||a.room||a.name.includes('rebirth')||a.name.includes('commandment'))continue;for(let i=0;i<130;i++){const p=a.polygon,j=Math.floor(rnd()*p.length),u=rnd(),v=p[j],b=p[(j+1)%p.length],cx=p.reduce((s,q)=>s+q[0],0)/p.length,cy=p.reduce((s,q)=>s+q[1],0)/p.length,t=.80+rnd()*.15;const x=cx+(v[0]+u*(b[0]-v[0])-cx)*t,y=cy+(v[1]+u*(b[1]-v[1])-cy)*t;const current=at(x,y);if(!current||current.material===M.water||current.material===M.lava||current.material===M.snow)continue;entity('prop_static','margin_detail_'+node,[x,y,surface(x,y)],{model:details[i%4],solid:'0',rendercolor:'155 161 139',renderamt:'255',skin:i%4===1?'3':'default'},`0 ${rnd()*360} 0`,.7+rnd()*.6);}}
// Reuse native paving/lava materials on thin authored floor patches; native ground
// remains underneath. This avoids path-edge encoding leaking stripes into water.
for(const a of terrainPaint){if(a.material===M.water||a.material===M.grass||a.material===M.snow)continue;const p=inset(a.polygon,a.name==='paving'?1:.92),z=Math.ceil(a.z/128)*128+(a.name==='paving'?16:8);prism(p,z,z-160,a.material===M.stone?M.paving:a.material,M.rock,'235 235 235 255');}
for(let i=0;i<300;i++){const x=-15400+rnd()*30800,y=-15000+rnd()*30000;if(at(x,y)||at(x+220,y)||at(x-220,y)||at(x,y+220)||at(x,y-220))continue;entity('prop_static','ridge_rock_'+i,[x,y,surface(x,y)],{model:'models/props_nature/river_rocks00'+(1+i%3)+'.vmdl',solid:'0',rendercolor:'106 118 119',renderamt:'255'},`0 ${rnd()*360} 0`,2+rnd()*3);}
...
entity('info_player_start_goodguys','player_start',[X(158),Y(51),550]);entity('info_player_start','editor_start',[X(158),Y(51),550]);entity('info_player_start_badguys','enemy_start',[X(30),Y(48),550]);entity('ent_dota_game_events','game_events',[0,0,0]);
for(let i=0;i<4;i++)entity('info_target','player_'+i+'_builder_spawn',[X(154+i*3),Y(51),550]);
...
grid=grid.replaceAll('radiant_summer_basic.vmap','radiant_ti10_basic.vmap');
...
grid=grid.replace(/^(\t{6})"([^"]+)" "(\w+)_array"\s*\[([^\]]*)\]/gm,(all,indent,k,t,body)=>{
 ...
 if(k==='verticesHeight'||k==='verticesWater')for(let y=0;y<=128;y++)for(let x=0;x<=128;x++){const wx=-16384+x*256,wy=-16384+y*256;a[y*129+x]=k==='verticesHeight'?heights[y*129+x]:(at(wx,wy)?.material===M.water?1:0);}
 if(k==='gridnavFlags')for(let y=0;y<512;y++)for(let x=0;x<512;x++){const wx=-16384+x*64+32,wy=-16384+y*64+32,b=at(wx,wy);a[y*512+x]=b&&b.material!==M.water&&!blockedAreas.some(p=>inside(wx,wy,p))?0:1;}
 if(k==='blendOpacity'||k==='grassOpacity'||k==='blendTransitionAndPath')for(let y=0;y<=1024;y++)for(let x=0;x<=1024;x++){const b=at(-16384+x*32,-16384+y*32),stone=b&&(b.material===M.paving||b.material===M.stone),dirt=b&&b.material===M.dirt;let v=k==='blendOpacity'?(dirt?'200 255 0 128':'0 255 0 128'):k==='grassOpacity'?(b&&!stone&&!dirt&&biomeAt(b)===0?210:0):(stone?'0 0 0 0':'0 255 0 0');a[y*1025+x]=v;}
