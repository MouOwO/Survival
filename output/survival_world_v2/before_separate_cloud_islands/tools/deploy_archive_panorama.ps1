...
$files += @('styles/custom_game/archive_kit.css', 'scripts/custom_game/ui_layers.js')
$kit = Join-Path $source 'images/custom_game/archive_kit'
$assets = @(Get-ChildItem -LiteralPath $kit -File -Recurse | ForEach-Object { 'images/custom_game/archive_kit/' + $_.FullName.Substring($kit.Length + 1).Replace('\','/') })
