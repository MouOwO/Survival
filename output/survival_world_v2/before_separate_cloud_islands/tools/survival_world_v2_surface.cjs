...
for(let cy=0;cy<256;cy+=chunkCells)for(let cx=0;cx<256;cx+=chunkCells){const verts=[],faces=[],ids=new Map(),boundary=new Map();
 ...
 for(let gy=cy;gy<cy+chunkCells;gy++)for(let gx=cx;gx<cx+chunkCells;gx++){const x=-16384+(gx+.5)*gridStep,y=-16384+(gy+.5)*gridStep;if(nativePreserved(x,y))continue;const a=vertex(gx,gy),b=vertex(gx+1,gy),c=vertex(gx+1,gy+1),d=vertex(gx,gy+1);const slope=Math.hypot(terrainHeight(x+64,y)-terrainHeight(x-64,y),terrainHeight(x,y+64)-terrainHeight(x,y-64))/128,m=paletteNames.indexOf(slope>.9?'ridge':localTheme(x,y));triangle([a,b,c],m);triangle([a,c,d],m);}
...
fs.writeFileSync(path.join(OUT,'surface_design.json'),JSON.stringify({mainIsland:'four rotated original U islands',sourceMap:'template_map.vmap',crossCenter:originalIsland.center,crossLength:originalIsland.levels.crossHalfLength*2,crossWidth:originalIsland.levels.crossHalfWidth*2,groundSurfaceZ:396,waterZ:416,transitionWidth:760,architecture:'continuous closed heightfield',preserved,gridStep,meshAudit,canopyCount,groundRoots:groundRoots.map(a=>({name:a.name,theme:theme(a),z:level(a)+12})),lilySource:'Native tileset entities disabled'},null,2));
