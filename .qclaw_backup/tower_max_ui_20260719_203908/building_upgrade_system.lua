...
local function sync_tower_abilities(state, row)
    ...
    for _, ability_name in ipairs(row and row.active_skill_ids or {}) do
        local allowed = ability_name ~= "ability_upgrade_tower_max"
            or not row.rarity or row.rarity == "" or row.rarity == "N"
        if allowed then
            wanted[ability_name] = true
            local ability = state.unit:FindAbilityByName(ability_name)
                or state.unit:AddAbility(ability_name)
            if ability then ability:SetLevel(1); ability:SetActivated(true) end
        end
    end
    for _, skill_id in ipairs(row and row.skill_ids or {}) do
        wanted[skill_id] = true
        local ability = state.unit:FindAbilityByName(skill_id)
            or state.unit:AddAbility(skill_id)
        if ability then ability:SetLevel(1); ability:SetActivated(true) end
    end
    for _, ability_name in ipairs({
        ...
        local ability = state.unit:FindAbilityByName(ability_name)
        if ability and not wanted[ability_name] then state.unit:RemoveAbility(ability_name) end
    end
    ...
    if state.tower_class then
        for _, class_data in pairs(state.definition.class_options or {}) do
            ...
            if ability then state.unit:RemoveAbility(class_data.ability) end
        end
    end
end
...
local function upgrade_tower(state, mode)
    ...
    if not state.tower_class and state.level == 5 then set_class_buttons(state.unit, true) end
