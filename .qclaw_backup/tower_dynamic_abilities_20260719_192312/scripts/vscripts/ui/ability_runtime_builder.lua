...
local function tower_upgrade(state, resources)
    ...
    if not state.tower_class then
        ...
        local result = merge({
            ...
            status_text = data and "升级至箭塔Lv." .. tostring(state.level + 1)
                or "已达当前阶段上限",
    ...
    local row = tower_routes.get(state.tower_class, next_level - 5)
    if not row then
        return { available = 0, can_afford = 0, current_level = state.level, status_text = "已达最高等级", tower_class_name = state.tower_class_name or "" }
    end
    ...
    local result = merge({
        ...
        current_level = state.level,
        next_level = next_level,
        status_text = "升级至" .. tower_routes.display_name(row),
        tower_class_name = state.tower_class_name or "",
        tower_name = tower_routes.display_name(row),
        skill_ids = row.skill_ids,
    }, cost_data(cost))
    return with_affordability(result, cost, row.population_delta or 0, resources)
end
