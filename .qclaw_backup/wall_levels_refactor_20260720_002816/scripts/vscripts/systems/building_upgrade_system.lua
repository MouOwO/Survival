...
local function sync_tower_abilities(state, row)
    ...
    -- 先释放有限的原生技能槽。转职瞬间塔仍持有七个转职按钮，
    -- 如果先 AddAbility，路线被动会因槽位已满而添加失败。
    ...
    local function add_wanted(ability_name)
        local ability = state.unit:FindAbilityByName(ability_name)
        ...
        if not ability then
            logger.error(
                ...
                "AddAbility failed: " .. tostring(ability_name)
                    .. " tower=" .. tostring(state.unit:entindex())
            )
            return false
        end
        ability:SetLevel(1)
    ...
    for _, ability_name in ipairs(row and row.active_skill_ids or {}) do
        if wanted[ability_name] then add_wanted(ability_name) end
    end
    for _, skill_id in ipairs(row and row.skill_ids or {}) do
        add_wanted(skill_id)
    end
