...
local DEBUG_ATTACK_RATE_MODIFIER = "modifier_debug_fixed_attack_rate"
...
local function add_speed(context)
    local hero = PlayerResource:GetSelectedHeroEntity(context.player_id)
    if not hero or hero:IsNull() then
        return false, "hero_not_ready"
    end
    hero:SetBaseAttackTime(0.1)
    hero:RemoveModifierByName(DEBUG_ATTACK_RATE_MODIFIER)
    local modifier = hero:AddNewModifier(
        hero, nil, DEBUG_ATTACK_RATE_MODIFIER, {}
    )
    if not modifier then
        return false, "fixed_attack_rate_modifier_failed"
    end
    notify(context, "固定攻击间隔已设为0.1秒（目标10次/秒）")
    return true
end
