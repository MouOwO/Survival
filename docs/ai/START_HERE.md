...
- 当前插入任务（2026-08-12，生产实现与自动验证完成）：Modifier注册改为每个脚本启动generation完整链接一次、同代去重；普通`addhero`零链接检查Lua类，缺类时定向重载并重新链接该模块全部声明，失败则在替换事务开始前中止。小地图新增显式`materials/overviews/template_map.vtex`，VMAT运行时依赖已由自动哈希纹理切换为稳定VTEX，旧哈希产物已删除。废弃血条链、猴王饰品序列和Mango Tree项目粒子回退保持上一轮修复。9项Lua测试、7/8项PowerShell测试、14个Lua语法、资源编译/依赖、UTF-8及限定diff通过；唯一失败是既有无关本地化缓存契约。下一步Workshop Tools完全冷启动确认小地图与Modifier，再热重载连续`addhero`，不得把自动测试记为实机验收。
