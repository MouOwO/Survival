...
- 当前插入任务（2026-08-08，已按用户澄清纠正）：城墙固定锚点实现保持不变。人口训练权威CSV恢复原始数据且不再修改：阶段1至5各`max_count=5`，阶段6为`max_count=100`并启用。运行时按team共享、按`training_id`独立计数，当前CSV阶段达到自身上限后才顺序加载下一阶段；农场等级只校验前置条件。服务端和Tooltip共用CSV递增费用与阶段内进度。专项自动验证完成后，下一步Workshop Tools实测训练1首次加载、1到2切换、费用递增及阶段6行为；尚未用户验收。
- - 当前插入任务（2026-08-07）：城墙基础Hull为256；用户实测并确认波次怪角色Hull为普通32、精英64、Boss无碰撞，`scalemonster <倍数>`按角色基准不累计作用于当前及之后波次怪且不改模型；研究所/农场施工、实际视觉及KV回退与英雄祭坛统一为`radiant_ancient001.vmdl`缩放0.34，金矿保持原生缩放1。专项Lua 5.1/契约、目标Lua语法和diff检查通过；单位模型旧契约仍被无关伐木工CSV缺项阻断，下一步Workshop Tools实测真实碰撞与模型尺寸，尚未用户验收。
- 前序六项玩法修复（2026-08-07）：无声金矿飘字、农场/金矿1/5上限、CSV首发顺序和农场Tooltip实现保持完成；其中金矿`0.0875`及城墙Hull基准128已被上方用户最新要求分别替换为模型缩放1和Hull 256。其他专项验证结果保持有效，仍待Workshop Tools合并验收。
