...
- 建造密令的 CSV 描述原本要求发放道具，但旧 handler 只增加一次自动免费升级额度。现改为给当前玩家的建造者发放 `item_survival_rogue_building_upgrade`，在背包选中道具后指定自己的建筑。
- 发放失败（无建造者或背包已满）保留奖励选择；道具使用时重新校验所有权、存活、建造完成和原有升级条件。主城、城墙、民居、防御塔和金矿都走已有服务端升级入口；成功开始一级免费升级才消费道具，失败不消费。已发出的旧局免费额度消费兼容逻辑保留。
- 核弹原本已向建造者发放道具，使用时分批清除当前非 Boss 敌方波次/挑战怪。虚弱宝珠、幸运怀表、渴血药剂等名称不代表背包道具：虚弱宝珠和渴血药剂是直接效果；幸运怀表、地精复制器、爆发药剂仍因效果/流程未定义而停用。训练靶子直接在城墙前生成目标单位。
- 本轮验证：Lua 5.1 模拟已覆盖物品发放、背包满、跨玩家目标、满级/施工目标、失败保留、成功后消费、金矿免费升级及核弹分批清怪/Boss排除。6 个相关回归文件和目标 Lua 语法检查通过；奖励 CSV 契约通过。全量 CSV 形状检查仍受既有 building_challenge_definitions.csv 列数不齐阻断。Workshop 背包图标/指定目标操作未验证，不能将模拟视为实机验收。
## 2026-09-25：本局宝物记录

- 顶部“宝物”入口展示本局最近 20 次成功肉鸽选择，按最新在前排列；服务端 `rogue_reward_service` 和 `treasure_history.js` 均保留 20 条。这里是选择历史容量，不是新增奖励种类，也不限制玩家继续领取。
- 历史快照沿用 `survival_rogue_reward`，从权威卡牌配置投影 `icon_name`、名称与说明；失败或重复选择不会新增记录，不同玩家分别维护。
- 宝物窗采用蓝黑底、金边、5×4 卡格，展示技能/道具图标、最新标记和悬停效果；复用 `SurvivalUILayers`，支持窗口外点击及 Esc 关闭。
- `tools/test_treasure_history.js` 验证容量、数字键排序、图标、玩家隔离和关闭；`scripts/vscripts/tests/test_rogue_reward_history.lua` 验证服务端裁剪、顺序、图标投影、失败和重复请求。UI 资源已通过 Resource Compiler；实机布局仍需 Workshop 验收。
