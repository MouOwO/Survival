...
## 当前角色检查点（2026-08-24）：数据库工作切换为 AI CTO 架构审查

- 后续涉及数据库、HTTP API、永久档案、奖励、在线 session、订单或事务时，先读取 `docs/ai/AI_CTO.md`，按 Architecture Review -> API Contract -> Database Impact -> Implementation -> Testing Matrix 推进，禁止直接从局部 SQL 或 Lua 补丁开始。
- 当前真实链路是 `Dota server Lua -> loopback Python API -> Supabase RPC/PostgreSQL`；Python 当前并非 FastAPI，而是 `ThreadingHTTPServer + FishingApplication + SupabaseRpcClient`。手册提供目标职责和审查原则，实际调用链以仓库代码为准。
- CSV 继续是业务配置权威源。数据库问题优先核对 CSV/生成 Lua/Python payload/RPC 签名/migration/远端函数版本/事务日志的完整一致性。
- 当前尚未收到新的具体数据库故障；不要修改业务代码。收到问题后先输出手册要求的架构、接口、数据库影响、风险和测试矩阵，再决定实施范围。

## 最新修复检查点（2026-08-24）：城墙毁坏直接触发 final

- 已将在线最终结算接入已完工城墙死亡的失败闩锁分支：`finish("wall_destroyed")` 先于 `SetGameWinner(DOTA_TEAM_BADGUYS)` 执行；不模拟 `player_disconnect`。
- `POST_GAME` 与 `game_end` 保留为幂等兼容入口。下一次实机只需让已完工城墙被毁，优先核对 `game_end_final_requested source=wall_destroyed`、`checkpoint_request_started ... final=true`、API HTTP 200 和唯一一次 `session_closed`。

## 最新修复检查点（2026-08-24）：POST_GAME 已接入 final，待实机复验

- 新 API 日志确认终局前三次普通 checkpoint、星之庇佑 grant 和档案刷新均成功；进入 `POST_GAME` 后没有 final 请求，因此此前断点已定位到游戏终局入口，而不是 Supabase 或奖励事务。
- `game_rules_state_change` 现在在 `DOTA_GAMERULES_STATE_POST_GAME` 调用 `online_time_service.finish("game_rules_state_change")`；原 `game_end` 入口保留为兼容兜底，双入口复用现有 session 状态保证 final 幂等。
- 契约、Lua 5.1 行为、目标语法、严格 UTF-8 和限定 diff 检查通过。下一步唯一动作是 Workshop Tools 完全冷启动后正常结束一局，核对 `game_state_post_game`、`game_end_final_requested source=game_rules_state_change`、`checkpoint_request_started ... final=true`、API HTTP 200 和唯一一次 `session_closed`。
