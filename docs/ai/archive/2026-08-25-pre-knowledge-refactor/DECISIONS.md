...
## 2026-08-23：多人最小闭环优先于正式商品支付

- 决定：按照“一个问题解决并验收后再进入下一个问题”的门禁顺序推进，先完成数据库迁移核对、主机冷启动、双玩家加入和双玩家档案/玩法隔离，再扩展四玩家，最后实施正式商品与支付。
- 决定：数据库工作继续推进，但当前只实现直接服务多人核心闭环的账号、session、在线累计、奖励、结算、幂等和恢复能力；未确认商品类型、订单、退款和撤销语义前，不新增正式支付发货 schema。
- 原因：多人房间、状态归属和断线恢复属于高返工风险基础；先做支付会在身份、权益归属和结算模型未稳定时形成重复实现。API 继续绑定 `127.0.0.1`，不把 HTTP 暴露误当作 Dota 房间联机方案。