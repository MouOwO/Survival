...
## 2026-08-24：失败局直接从城墙毁坏入口结算

- `POST_GAME`/`game_end` 在 Workshop Tools 中不能作为失败局唯一回调；已完工城墙死亡的 Survival 业务失败分支现在直接调用 `online_time_service.finish("wall_destroyed")`，随后才设置坏人方胜利。
- 该入口尚未完成本轮 Workshop Tools 实机复验。自动检查只能证明 Lua 调用关系和 final 幂等逻辑，不能证明 HTTP callback 或后端持久化。

## 2026-08-24：Workshop Tools 的 `game_end` 不能作为唯一终局入口

- 实机证据显示游戏已进入 `DOTA_GAMERULES_STATE_POST_GAME`，但未观察到已注册的 `game_end` Lua callback；同局此前普通 checkpoint、奖励和档案刷新均正常，因此不能继续把 `game_end` 当作在线 session finalization 的唯一入口。
- 当前规避是使用已经触发的 `game_rules_state_change -> POST_GAME` 调用幂等 `online_time_service.finish()`，同时保留 `game_end` 兼容入口。自动测试证明双入口不会重复发送 final，但 Source 2 实际事件顺序和 final HTTP callback 仍需 Workshop Tools 冷启动复验。
...
0. **`.cline/local-toolchain.json`中的绝对路径会随工作区迁移或MSYS目录布局漂移。**
   - 2026-08-12旧配置仍指向不存在的`D:\steam`、旧用户WorkBuddy Python及`C:\msys64\mingw64\bin`；实际规范工作区位于`E:\steam`，有效Lua/Luac 5.1.5位于`C:\msys64\msys64\bin`，Python 3.14.3位于当前用户WorkBuddy目录。配置已按本机实测修正。
