...
## 在线 session 终局事件边界（2026-08-24）

- 失败局的权威业务入口是已完工城墙死亡并通过 `building_defeat_rules` 闩锁确认的分支；该分支先调用 `online_time_service.finish("wall_destroyed")`，再调用 `SetGameWinner`。不能把失败局建模为 `player_disconnect`。
- Workshop Tools 实机可进入 `DOTA_GAMERULES_STATE_POST_GAME` 而不观察到已注册的 `game_end` Lua callback，因此在线 session 最终结算必须由 `game_rules_state_change -> POST_GAME` 覆盖；`game_end` 只作为兼容入口，不能成为唯一入口。
- 胜利和失败都调用同一个幂等 `online_time_service.finish(source)`。第一次终局入口将活动 session 迁移到 FINALIZING 并发送或排队 `final=true` checkpoint；后续终局入口不得重复累计、重复发奖或重复关闭 session。
- `/health`、普通 checkpoint 和奖励后的 `/v1/profile` 成功都不能替代终局验收。完整证据必须包含 POST_GAME 入口、final HTTP 200、Lua callback 和唯一一次 `session_closed`。
