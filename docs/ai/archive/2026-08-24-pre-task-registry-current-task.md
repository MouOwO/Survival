## 当前工作模式（2026-08-24）：数据库架构审查优先

- 用户要求后续数据库工作先采用 AI CTO/架构师视角，已阅读 `docs/ai/AI_CTO.md`。任何数据库改动必须先完成 Architecture Review、API Contract、Database Impact、错误恢复与 Testing Matrix，再进入实现。
- 当前真实边界为 `Dota server Lua -> 127.0.0.1 Python API -> HTTPS Supabase RPC -> PostgreSQL`。Panorama 不访问 HTTP，Lua 不访问 SQL，Python 是唯一数据库入口；玩家永久身份由 Python 使用稳定 pepper 做 HMAC-SHA256 后入库。
- 手册描述 Backend Gateway 为 FastAPI，但当前生产实现实际使用 `ThreadingHTTPServer + FishingApplication + SupabaseRpcClient`。后续设计遵守手册的职责边界，不得把尚未采用的 FastAPI/router/repository 目录当作当前事实，也不得为对齐手册擅自重构后端。
- `data/csv/玩家档案系统/` 继续是玩法属性、奖励定义、checkpoint/lease、definition version 和 profile schema version 的权威源；数据库与 Python 不得维护未受 CSV 约束的第二套业务默认值。
- 当前数据库主链为 `/v1/profile` 与 `/v1/online-time/checkpoint`，核心持久化对象为 `survival_players`、`player_gameplay_stats`、`online_time_sessions`、`online_time_idempotency`、`reward_grants`、`player_effect_totals` 和版本化奖励定义。主要风险不是单一 SQL CRUD，而是远端 migration 状态、跨层幂等身份、终局 final 请求、事务阶段日志和端到端可观测性。
- 当前没有新的具体数据库故障需求，本轮不修改 Gameplay、Python、SQL、CSV 或生成文件。下一项数据库问题必须先给出 Architecture Summary、File Impact、Sequence Flow、API Contract、Database Impact、Implementation Plan、Risk Analysis、Testing Matrix 和 Review Checklist。

## 本轮修复（2026-08-24）：城墙毁坏直接触发在线最终结算

- 用户实机确认进入 `POST_GAME` 后仍未观察到 Survival final 回调，因此不再把引擎终局事件作为失败局唯一入口。
- 已在已完工城墙死亡、失败闩锁确认后的业务分支中先调用 `online_time_service.finish("wall_destroyed")`，再调用 `GameRules:SetGameWinner(DOTA_TEAM_BADGUYS)`；施工城墙、主城和重复城墙死亡不改变原有失败条件。
- 该入口是全局失败语义，不调用按玩家处理的 `disconnect()`；所有活动玩家仍通过统一 `finish()` 发送或排队 `final=true`，后续 `POST_GAME`/`game_end` 入口依靠 session 状态幂等抑制重复 final。
- 自动验证通过：`ONLINE_TIME_POST_GAME_CONTRACT_PASS`、`ONLINE_TIME_DEBUG_CHECKPOINT_LUA51_PASS`、`BUILDING_DEFEAT_RULES_LUA51_PASS`、目标 Lua 5.1 语法、严格 UTF-8 和限定 `git diff --check`。Lua 行为覆盖直接 final、在途普通 checkpoint 排队 final 以及后续 `POST_GAME`/`game_end` 不重复发送。
- 两项较宽的七塔融合回归被既有过期断言阻断：测试仍要求 `tower_fusion_runtime.csv` 提供 `max_count_per_player=5`，但当前权威 CSV 和生成 Lua 均无该字段；未修改无关权威配置迎合旧测试。仍需 Workshop Tools 冷启动确认城墙毁坏后立即出现 `source=wall_destroyed`、单次 `final=true` HTTP 请求和 `session_closed`。

## 本轮修复（2026-08-24）：POST_GAME 触发在线 session 最终结算

- 用户提供的新日志确认：终局前连续三个普通 checkpoint 均返回 HTTP 200，并分别产生星之庇佑 grant 和 `/v1/profile` 刷新；进入 `DOTA_GAMERULES_STATE_POST_GAME` 后没有第四个 final checkpoint，也没有 `session_closed`。因此断点已收敛到游戏终局入口，Supabase、普通 checkpoint、奖励和档案刷新不是本次失败点。
- 根因是 `addon_game_mode.lua` 虽已监听可靠出现的 `game_rules_state_change`，但只处理 `GAME_IN_PROGRESS`；在线结算唯一依赖本次 Workshop Tools 路径未观察到的 `game_end` 事件。
- 已在 `DOTA_GAMERULES_STATE_POST_GAME` 分支调用 `online_time_service.finish("game_rules_state_change")`，并保留 `game_end -> finish("game_end")` 作为兼容入口。`finish()` 记录入口来源；现有 session 迁移保证两个终局事件都到达时不重复发送 final checkpoint。
- 自动验证：`ONLINE_TIME_POST_GAME_CONTRACT_PASS`、`ONLINE_TIME_DEBUG_CHECKPOINT_LUA51_PASS`、目标 Lua 5.1 语法、严格 UTF-8 和限定 `git diff --check` 通过。仍需 Workshop Tools 冷启动确认 `game_state_post_game`、`game_end_final_requested source=game_rules_state_change`、`final=true` HTTP 200 和单次 `session_closed`；自动验证不等于引擎实机通过。
