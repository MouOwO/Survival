...
- 伤害计算顺序固定为：War3显示护甲先减固定减甲；只对剩余正护甲施加百分比无视，零/负护甲保持原值。正护甲承伤倍率为`1 / (1 + 0.02 × W)`；零/负护甲先按`DotaArmor = W / 3`投影，再用`1 - 0.052 × DotaArmor / (0.9 + 0.048 × abs(DotaArmor))`。项目怪物的正甲曲线与运行时Dota护甲投影是两个独立边界，不能混为一个公式。
