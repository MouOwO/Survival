...
- 公共技能`proto_meteor`已重做为五级“陨石坠落”：攻击命中12%概率在目标快照位置坠落，0.8秒后500范围爆炸并造成触发时全属性×3纯粹伤害；LV2起留下3秒熔岩，在落地后第1/2/3秒各造成×1；LV3起区域内唯一减速30%，离开全部区域立即移除；LV4无新增；LV5同点晚0.5秒落下第二颗，其爆炸和熔岩均为80%。每英雄从触发到最后一次熔岩结算维持独立锁，概率判定前跳过；LV1落地即解锁。视觉使用Lua控制的基础投射物坠落、基础爆炸和Viper Nethertoxin地面区域，伤害碰撞仍由Lua状态与Hull精确AOE权威决定。
