...
- 权威CSV当前为槽位数4、槽间距80、墙外偏移384、到位阈值24、退出阈值48、排队间距160并生成到`global_rules.lua`。`WALL_ENGAGEMENT_SLOTS_LUA51_PASS`覆盖相同槽位/队列点不重复移动、40范围漂移保持攻击、超过48单次恢复移动及等待怪死亡补位后只下达一次新移动；五个目标Lua 5.1语法、配置生成与`CheckOnly`、95个生成模块UTF-8及限定`diff --check`通过。既有城墙Hull测试因`building_system.lua`带UTF-8 BOM而被MSYS Lua 5.1在加载前拒绝，未进入断言，本次未修改Hull。仍需Workshop Tools冷启动观察攻击动画、四只持续攻击、第五只排队及死亡补位，自动验证不等于引擎实机验收。
