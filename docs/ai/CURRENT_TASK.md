## 当前修改（2026-09-26）：按用户要求完全撤销头顶称号

用户实机报告：开局左上角出现称号图，使用体验物品后游戏闪退。此次删除称号全部运行代码、物品定义、注册、前端、素材、编译产物及专用测试/构建目录，不保留开关或重做版本。经验及证据：docs/ai/TITLE_REMOVAL_POSTMORTEM_20260926.md。此前初级箭塔 LV1–LV5 的模型替换保留，详情 output/tower_models_20260926/REPORT.md。

## 当前修改（2026-09-26）：商店自动购买、挑战合成、头像与建筑预览

已完成本轮 1–14 项代码及资源修改；第 15 项只生成建筑/箭塔概念预览，没有替换模型。知识之书服务器每秒自动购买，保留手动无冷却并说明补货 10 秒；商店记住页签；52 个宝物缩略图绑定修复；冰刃专用新图标；技能选择界面青金风格。范围拾取显式调用 Claim，验证 9 个 LV1 自动变为 1 个 LV3。三个指定挑战等待 1 秒，罪渊移除立即激活旁路。六类挑战完整英雄头像及 Doom 路由、1043 个头像深色背景、Primal 镜头调整。9 项 Lua 与 6 项前端回归通过，四组资源编译 0 failed。旧 bootstrap 检查要求自动开局 true，与当前 false 不符而失败，本轮不更改开局设置。尚待完整重启 Dota 2 后新局实机验收头像观感。

详情与预览：output/polish_batch_20260926/REPORT.md、building_concepts.png；该目录包含备份、构建与测试日志。

## 当前修复（2026-09-26）：恢复顶部入口及七宗罪恐怖利刃

本轮截图明确为 challenge_10 七宗罪，上一轮处理的是 challenge_11 十宗罪，两者不同。顶部返回/宝物/福利/商店等六个入口撤去统一大图覆盖，恢复 icon_unify 修改前的原始绘制逻辑；保留资源、属性图标的新样式。

七宗罪的 challenge_monster_terrorblade_fractal_horns 原来只有 horns_arcana 头部；现 CSV 补入官方默认 wings（306）、weapon（307）、armor（476），出生 NPC 直接使用 terrorblade_arcana。实际日志有每个 horns_arcana prop_dynamic 的 no sequence named:idle；bone_merge 组件现在不设置独立 DefaultAnim，继承主体骨骼。保留原有 abysm 动作、粒子、模型缩放与所有战斗数值。

服务端肖像元数据及客户端精确白名单新增此资源；选中单位使用既有 ScenePanel 的完整 npc_dota_hero_terrorblade 动态基础头像（不向头像传饰品 ItemDef），等待快照时使用现有过渡遮罩。世界模型仍为至宝，头像为完整原生基础外观；保留多选与旧快照隔离。

四项 Lua、四项 Node 回归通过，新增七宗罪世界四件挂载/动作/清理及肖像身份、前端选择一致性/多选测试；四件模型 VPK 存在性、NPC/CSV 范围与语法检查通过。HUD 同步 content，13 compiled / 0 failed。旧 test_native_portrait_runtime_contract.ps1 在既有隐藏镜头编辑器的第二个 ScenePanel 计数处失败；布局本轮未改，不以此陈旧合同声称通过。未完成游戏内重开对局后的渲染验收。证据 docs/ai/validation/20260926/seven_sins_fix.json；备份 output/seven_sins_fix_20260926/before。

## 当前修复（2026-09-26）：罪渊缺件与原生动态头像

用户明确要求保留动态 3D。罪渊十层 NPC 初始主体与 archetype 对齐；为第 3、6、9 层补齐 9 个官方默认/至宝适配部件（共 52 件），修正帕吉皮肤与至宝动作标记，预加载按声明资源包选择并补充失败诊断。新增 portraits_custom.txt，覆盖 34 建筑/城墙、8 塔、10 工人，按实际模型取景，统一深青背景与柔和光照。五项 Lua 回归、多选头像、KV/语法与资源/视锥校验通过，旧配置缺件断言可复现。未做本轮实机渲染验收；须完整重启 Dota 2 后新局检查。详见 docs/MODEL_PORTRAITS_20260926.md；证据 output/model_portraits_20260926/validation.json。

## 当前修改（2026-09-26）：英雄拾取瞬时施法

范围拾取 `ability_survival_pickup_materials` 在现有零前摇基础上，增加IMMEDIATE、IGNORE_BACKSWING、DONT_CANCEL_MOVEMENT，施法动画设置ACT_INVALID。使用引擎即时施法路径保留原攻击/移动命令，不通过Stop或重新下达攻击命令模拟续攻。F键与地点/AOE选取不变，施法距离700、拾取半径300、冷却0.5秒及拾取结算保持原样。

本机Valve VPK存在 `item_seer_stone` 的POINT/AOE/IMMEDIATE组合，以及 `courier_morph` 的IMMEDIATE/IGNORE_BACKSWING/DONT_CANCEL_MOVEMENT和ACT_INVALID组合；参考保存在 `output/instant_pickup_20260926/installed_valve_ability_reference.json`。KV解析对比确认仅拾取技能的行为和动画两字段变化。现有Lua范围拾取回归通过（两类掉落、归属、边界、背包满和特效清理）。验证：`docs/ai/validation/20260926/instant_pickup.json`。需要重开Workshop对局使NPC KV生效；攻击动画/攻击计时连续性尚未实机验收。

## 当前修复（2026-09-26）：伐木攻击间歇卡顿

已修复伐木攻击成长触发的无关建筑全场扫描、英雄/金矿重算与工人攻速/射程重复设置；攻击成长仍逐次立即生效。局部成长事件携带字段范围，成长UI按玩家0.1秒合并；树木满级回血不再重复刷新全部工人的目标/效率。10项Lua回归及7份语法检查通过，5条旧版本回归可复现。未进行Workshop实机帧率验收，请重开对局复测。详情：`docs/LUMBERJACK_STUTTER_20260926.md`，验证：`docs/ai/validation/20260926/lumberjack_stutter.json`。
...
## 当前实施（2026-09-26）：图标材质润色与普通挑战怪物头像

已替换22张图标（4建筑、5资源/人口、13研究物品），暗青底、立体材质、柔和光影，256×256。普通挑战11入口按实际怪物配置映射头像，新增8张原生头像；保留用户认可的挑战建筑图标。CSV只改图标字段，生成Lua已更新。源文件与content同步，图标115项/HUD15项编译0失败；两项Node回归、CSV/Lua/UTF-8/同步检查及48px浏览器预览通过。顺带修复上一轮研究所本地化文本的管道编码损失。未完成Workshop实机验收。

说明：`docs/NATIVE_HUD_ICONS.md`；预览：`output/icon_polish_20260926/preview.png`；完整提示词：`docs/ai/validation/20260926/icon_polish_prompts.json`；检查：`docs/ai/validation/20260926/icon_polish.json`。

## 当前排障（2026-09-26）：偶发粒子渲染闪退

三份转储均指向 `particles.dll+0x42428` 空指针写入。四处特效清理已改为先撤销活动记录再调用引擎，防止重入重复释放及覆盖新状态；旧代码模拟复现，新代码四路径回归通过。另三项英雄预载/召唤回归通过；研究所双语名称及别名已补齐。具体native触发源仍UNKNOWN，需Workshop冷启动复测，不能把名称警告当作闪退根因。详见 `docs/CRASH_20260926.md`。

## 当前实施（2026-09-26）：原生图标、天赋与英雄饰品预加载

本轮15项已完成代码与资源同步：106张图标、47个技能映射，建筑/工人缩略图、研究/祭坛/商店/挑战原生素材、人口鸡腿、资源图标、工人等级名及范围拾取手套。建造师天赋未选择问号闪烁，选择后常驻对应缩略图；重复打开不重复发奖。农场升级改为本人主城门槛，避免同队城市越级。英雄召唤补齐账号饰品的玩家级异步预加载。

9项 Lua 5.1、4项 Node 回归及CSV逐列/图标引用检查通过；图标107项、HUD15项编译均0失败。未完成 Workshop 新局实机验收。说明见 `docs/NATIVE_HUD_ICONS.md`，记录见 `docs/ai/validation/20260926/native_hud_icons.json`。

## 当前实施（2026-09-26）：小地图右侧快捷操作

视觉追改完成：移除快捷按钮右侧名称与大底框，176px宽改64px；蓝胖图标下方“空格”，透明浅色城堡图标下方“F2”。原有输入行为保持，缩放避让与遮罩检查通过；SVG及HUD已同步content编译，验证记录 `docs/ai/validation/20260926/minimap_compact.json`。本次仅浏览器检查SVG外观，未宣称游戏内显示验收。

追加完成：F1 固定选择本人的正式英雄，存活时定位镜头；保留空格建造师与F2回程。已验证身份缺失/召唤失败替身、英雄更替、阵亡选择、输入及弹窗拦截、旧上下文与重复回调；HUD已同步编译15项、0失败。本追加未做实机验收，记录 `docs/ai/validation/20260926/hero_f1_selection.json`。

已完成代码与资源同步：建造师 [空格]、英雄回程 [F2] 两个可点击入口；空格始终选本人建造师并定位镜头，F2 使本人正式英雄返回自己主基地旁。修复原回程按队伍找主城的问题，复用召唤安全落点与既有回程技能冷却/挑战退出。

聊天/TextEntry、模态/销毁确认、未准入/已失败时拦截；身份缺失、阵亡及回程冷却时对应入口不可用。六项 Node、六项 Lua 5.1 回归通过，五个 Lua 文件语法通过，七个源文件同步 content，HUD 编译15项、0失败。本轮未完成 Workshop 新局实机验收。

详细契约见 `docs/MINIMAP_SHORTCUTS.md`，证据见 `docs/ai/validation/20260926/minimap_shortcuts.json`。

## 当前实施（2026-09-26）：建筑生产队列与建造者交互

已完成本轮代码、图标和 UI 修改，验证层级为 STATIC / CONTRACT / SIMULATION 与资源编译；本轮未完成 Workshop 新局实机验收。详细规则见 `docs/BUILDING_PRODUCTION.md`，基础检查记录见 `docs/ai/validation/20260926/building_production.json`，中间版容量 6 的检查见 `docs/ai/validation/20260926/production_queue_capacity_6.json`；最新六等待格、字体及箭头替换见 `docs/ai/validation/20260926/production_six_waiting.json`。

- 主基地独立 1 秒训练队列，1 项进行中＋6 项等待（用户最新明确确认，总容量 7）；初始 LV1–4，某级达到实际累计上限后立即按等级升序补位，不需重选。名额/人口/费用预约、生命周期清理与退款按玩家隔离。
- 科技加入相同形态的 FIFO 队列，单项 2 秒，等待不预扣，开始时付费；重复点击按级递进，队首等待资源或前置条件时保留顺序。左键及热键接同一权威入口。
- 右键切换自动；手动队列优先，自动在完成后完整等待 1 秒继续。共享高级研究所仍可同队使用，等级、价格、队列与自动标记均按查看者私有展示。
- 训练/普通及高级研究字号在前版基础上再增大约 22–25%，最小显示比例 0.575；长任务名两行与全文悬停，金额万/亿简写；当前任务＋6 个等待格统一，缩放后血条遮罩按实际像素范围处理，buff 避让实际面板高度。
- 升级图标为深青底金边双箭头：用户确认箭塔区别后，要求大箭头主体较 five_v4 缩小约20%，并同步到城墙第二个升级按钮。现用 one_v4/large_v5，8小/2大引用；城墙原来两按钮都指小图，已修复第二项。记录见 `docs/ai/validation/20260926/upgrade_large_wall.json`。
- 建造者零半径加永久 NO_UNIT_COLLISION，保留地面移动。脚下建造只忽略本人预览占位，自动走到安全施工点，再完整验证、扣费施工；取消/死亡/淘汰/归属变化/超时不扣费，不传送。

### 前轮实施（2026-09-25）：HUD、合体道具与终波挑战修复


用户提出的 11 项调整已完成代码及资源修改；验证级别为 STATIC / CONTRACT / SIMULATION，尚未完成本轮 Workshop 新局验收。

| 项目 | 本轮结果 |
| --- | --- |
| 金币、木材 | 顶栏改亮金钱币堆和棕色切口原木，复用 Panorama 矢量面板绘制 |
| 人口图标 | 用户选定鸡腿食物图标，透明底手绘PNG，保留顶栏32×32尺寸、数字对齐及已用/上限人口显示 |
| 资源、旗帜对齐 | 清除文字固定最小高度与旧偏移，图标和文字在同一行垂直居中 |
| 波次文字 | 显示“下一波次 N 倒计时 MM:SS”；末波不虚构下一波，进入挑战后显示挑战阶段 |
| 小地图怪物数 | 按每位玩家分别统计所属正式进攻怪，各自上限90；小地图上方只显示本人数量，包含进攻Boss，不计独立挑战怪 |
| 超限规则 | 用户确认每人90安全；本人第91只开始独立连续10秒，仅显示数字，降回≤90取消重置；到期仍>90只淘汰本人，其他玩家继续；全员失败才结束整局 |
| 建造者碰撞 | 创建后 SetHullRadius(0) |
| 宝物 | 最近成功获得记录4→20，5×4蓝黑金边卡格、真实图标、最新标记和详情；不限制继续获取奖励 |
| 升级图标 | 10个建筑升级技能统一参考图式金色叠层尖角箭头；透明背景、柔和金色光晕；升1级主体宽度约58%，箭塔直升5级约84% |
| 血条遮挡 | 血条/飘字降层；正常HUD、小地图、提示框覆盖区域隐藏世界标记；登记的模态窗口打开时隐藏全部世界标记，关闭后恢复 |
| 伐木工合体 | 保留8档配置门槛；LV3起有主城LV5与金币要求。补充失败提示并让自定义HUD直接派发权威融合事件，失败不扣费可重试 |
| 建造密令 | 原实现仅发下一次免费升级额度，现实际发给建造者背包；指定自己的可升级建筑免费提升1级，成功开始才消耗；保留旧局额度兼容 |
| 终波挑战 | 原整体挑战入口N3门槛导致N1/N2直接胜利；入口改N1，各挑战独立难度要求不变，最后小怪清空后进入挑战，多玩家结束及奖励保存完成后才结算 |

用户后续确认：上限与失败范围均按玩家独立计算。个人超限淘汰复用现有 multiplayer_player_service，清理本人单位与出怪通道、取消延迟召唤/挑战并结束本人在线计时；其他玩家继续。失败者保留观战和私有UI，禁止继续发起玩法操作；主城/研究/工人HUD与英雄召唤缓存均按玩家隔离，避免同队受影响。补充24项Lua5.1行为回归与HUD界面回归通过，38个Lua/测试文件语法及UTF-8检查通过；HUD两项强制编译0失败。补充验证记录见 validation/20260925/per_player_wave_limit.json。

2026-09-26 图标追改：按用户参考图替换为金色上下叠层尖角，内置 imagegen 生成两张透明PNG，普通版主体宽度58.1%、直升5级版84.3%，柔和金色光晕。9个普通升级与1个直升5级引用保持；仅资源替换，无玩法变动。2 PNG和资产XML已同步content，定向编译3项0失败；旧SVG存入备份后移出作者目录。提示词及验证见 validation/20260926/upgrade_icon_prompts.json、upgrade_chevron_icons.json；本轮未做实机视觉验收。

2026-09-26 人口图标：用户从六款候选中选定鸡腿，透明手绘PNG接入现有top_population分支，仍按32×32垂直居中展示，人口数字与逻辑保持。JS、独立资源XML和PNG已同步content，定向编译3项0失败，波次HUD及刷新检查通过。记录与提示词见 validation/20260926/population_food_icon.json、population_food_prompt.json；未做本轮实机视觉验收。

道具卡审计：建造密令、核弹实际发背包道具；虚弱宝珠和渴血药剂直接应用效果，训练靶子直接生成单位。地精复制器、幸运怀表、爆发药剂仍为原配置未启用，未擅自开放或补造未确认玩法。

原11项验证：12个Lua5.1行为脚本、8个Node界面脚本通过；主HUD强制编译15项、0失败，宝物及箭头独立编译通过；源文件已同步相邻content。只读本机控制台可回显，但Lua探针没有GameRules上下文，因此没有据此声称新局视觉、背包操作、真实碰撞或完整挑战实机通过。需完整重启Workshop Tools并新开局，不能用旧局验收新Lua/KV。

已有问题保留：tools/aliyun_game_test_auth.py 在本轮开始已存在CRCRLF改动，使全局diff --check报空白错误；本轮限定范围通过。building_challenge_definitions.csv部分行省略末尾可选空字段，阻断既有全量严格CSV形状检查；不影响本轮定向生成及运行配置，未扩展修复。云端bundle及其哈希未变，挑战入口阈值不参与远端结算。

各域备份：output/hud_fixes_20260925/、output/treasure_upgrade_20260925/、output/units_rewards_fix_20260925/、output/wave_endgame_fix_20260925_223557/。可跟踪验证摘要见 validation/20260925/hud_gameplay_fixes.json。未提交或推送。
