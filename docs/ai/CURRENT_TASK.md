## 当前实施（2026-09-24）：有效改动筛选、临时文件清理与提交

- 保留地图源码/VPK、落点与导航修复、采木及 UI 性能优化、箭塔销毁/重建、蓝胖建造者、回归测试和必要文档。16 个贴图与 1 个布局仅重编译差异，源图/布局一致，已还原这 17 个产物。
- 删除 133 个确认可重建的未跟踪临时文件，共 1,459,367,161 字节；含 DMX 转换中间件、已完成补丁候选图、资源审计提取副本和 Python 字节码。保留当前地图、before 备份、美术源、最终验证记录/截图、部署环境与运行中的工具缓存。
- 28 项 Lua、2 项 Node 回归、37 个 Lua 语法检查通过。主图源码和 VPK 的 323 个资源 CRC 一致。旧 bootstrap 的 `auto launch missing` 断言在 HEAD 同样失败，记录为已有检查问题。
- content `master` 已提交 `f0d8380`；game 的有效代码/资源与本记录一同提交到 `dev`。本次没有 pull、重编地图或远端 push。明细见 `docs/ai/validation/20260924/branch_cleanup.json`。

## 前一阶段（2026-09-24）：建造者使用蓝胖基础模型

- 建造者 CSV 和单位 KV 的模型统一为 `models/heroes/ogre_magi/ogre_magi.vmdl`，缩放为 1；使用现有配置生成器同步 `builder_definitions.lua`。
- 现有启动预载直接读取建造者配置，UI 跟随实际单位模型。配置生成及 Lua 语法检查通过；需重启 Workshop Tools 后开新局查看，未做实机视觉验收。

## 前一阶段（2026-09-24）：防御塔销毁、立即腾格和建造入口恢复

- 销毁先释放建筑登记、占格、基础/路线名额与人口，再处理死亡表现；手动消除无需等待迟到的引擎死亡事件。重复事件幂等，旧塔不会清除新塔格子。
- 主体与挂件保留原生死亡活动，取消碰撞、选敌、控制和血条，4 秒后一次性清理；未新增每塔 update。死亡实体不会被异步外观/升级/属性刷新重新登记。
- 基础箭塔身份显式登记，少于 7 座恢复建造技能，达到 7 座关闭；移除融合后永久隐藏基础塔入口的旧逻辑。各路线与终极塔自身限额保持。
- 10 项本地回归通过，覆盖死亡时立刻原位重建、重复事件、外观异常、人口/路线预留释放及升级/攻击兼容。本轮未运行实机对局，逐模型死亡动作观感未验证；详见 `docs/TOWER_DESTRUCTION.md`。没有重编地图或提交推送。

## 前一阶段（2026-09-24）：采木/防御塔卡顿优化与楼梯导航

- 用户补充：约 7 个以上伐木工、4 个以上塔时卡顿，未做停采木对照；怪物卡在低过道转向高台/楼梯位置。
- 已修正树攻击证据的全局重复清理及销毁/实体索引复用边界；塔索敌稳定状态去掉重复写与练功攻击重复扫描，击杀后仍立即换敌。
- 和平采木判断同帧复用全场扫描；收益逐次结算。技能/研究 UI 合并为资源所有者的 0.1 秒单次刷新；商店窗口固定截止，HUD 有 dirty 才排任务。血条保留 0.1 秒检测，但相同数值不投影/不重复发网表。
- 当前 source 中 203 个阶梯导航修复未进入旧游戏 VPK，是卡角的明确地图证据。本轮已直接重新编译当前 source（318 成功、0 失败），323 个包资源 CRC 与源图匹配；未恢复飞行、未重建地形。
- 21 项本地回归已通过。实机 395 个落点、272 条 marker 路径及隔离塔攻击/换敌/禁树/idle 测试通过。另修正原生移除建筑后缓存引用失效实体的重复报错（本地回归通过，修复后实机待复测）。实际 FPS/内存改善及树木血效仍需实机对照，不将模拟次数当作帧时间。详见 `docs/COMBAT_PERFORMANCE.md`、`docs/MONSTER_NAVIGATION.md` 与 `docs/ai/validation/20260924/combat_performance.json` 的最终验收状态。
- 新增可选 15 秒服务器调用统计与只读树木受击 API 探针；默认不运行、不修改存档或单位。现有地图/资源以及其他未提交改动保留，没有提交或推送。

## 历史实施（2026-09-19）：双仓库清理与提交交付
