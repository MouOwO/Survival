...
N1 起清空最终波后先记录通关，再进入存档挑战阶段；不再跳过 N1/N2 的挑战阶段。
清空最终波不会先调用天辉胜利；如果挑战服务或出生点暂未就绪，会每秒重试，
...
建筑整体从 N1 开放，具体技能仍按各自难度条件解锁；N1 可挑战虚空之影1及好基友、前女友、瑞兽，N2 再开放虚空之影2。
...
入口仅在存档挑战2，通关后 N1 起生成挑战建筑，具体无尽组仍由无尽配置决定。每位玩家每局成功开启一次；与其他存档挑战互斥。所有难度使用同一组1000波属性，每波5只普通怪、60秒击杀时间、每秒攻击一次，统一模型。全灭后下一调度帧立即出下一波；超时清除剩余怪物并保留已完成波次积分，断线/结束存档挑战同时清理。
...
## 进攻怪数量与超限判负（2026-09-25）

`global_rules.csv` 的 `wave_alive_limit=90` 与 `wave_overflow_grace_seconds=10`
是服务器权威规则。每位玩家按进攻怪所属 `player_id` 独立计数，含普通怪、领头怪及进攻 BOSS，
不含练功房、建筑挑战与通关后的独立挑战；未生成怪物和尸体不计入。
四位玩家各 80 只时都安全，全场合计 320 不触发超限。

90 只安全，本人第 91 只起进入独立连续 10 秒宽限，期间保持正常出怪。本人回到 90 只及以下
即清除自己的倒计时；再次超过 90 从完整 10 秒重新计时。他人的增减不影响本人计时。
到期仍超限，复用 `multiplayer_player_service.defeat(player_id, "monster_limit_exceeded")`
只淘汰该玩家，关闭其出怪通道、清理所属单位并结束其在线计时。其他玩家继续对局和在线计时。
仅当全部参赛玩家均失败才全局判负。时间采用游戏时钟，暂停不消耗宽限。

全局 `wave_system` 的 `state.alive` 仍用于末波清怪与挑战阶段结算，不能用于个人上限。
`WAVE_STATE_GET_REQUEST {player_id=id}` 返回该玩家数量与倒计时；无玩家参数仅返回全局数量，
没有全局超限倒计时。私有事件 `survival_ui_private_snapshot` 的 `wave` 投影按收件玩家读取
`player_id`、`alive`、`alive_limit`、`overflow_active`、`overflow_remaining`、
`overflow_grace_seconds`、`player_defeated` 与 `defeat_reason`。UI 只消费服务器数据，
个人状态缺失时不回退到全局数量；败者显示“本局失败 / 可继续观战”，不弹全局结束画面。
断线仅清除所属进攻怪和计时；正常通关、游戏终局和重新初始化均取消相应超限计时。

SIMULATION：`scripts/vscripts/tests/test_wave_population_endgame.lua` 覆盖真实生成路径的
四人独立数量、90/91边界、相互独立倒计时、本人回落与他人死亡、断线/无效实体/清场、个人败后
其余玩家继续出怪及末波挑战、全员败只结算一次，以及原有 N1/N2/N3 终波挑战与奖励保存。
`test_personal_wave_projection.lua` 验证同队四人私有投影与重同步隔离，界面测试验证败者观战提示。
`test_player_elimination.lua`、`test_player_challenge_cleanup.lua` 与 `test_hero_summon_player_isolation.lua`
另验证个人在线结算、所属挑战清理、同队召唤缓存隔离、迟到回调及幸存者继续操作。
尚需 Workshop Tools 新局验证多人小地图计数/倒计时、个人淘汰观战与通关后挑战建筑可操作性。
