(function () {
    ...
    function image(parent,file){var i=$.CreatePanel("Image",parent,"");i.SetImage("file://{images}/custom_game/main_hud_v1/"+file);i.hittest=false;return i;}
    ...
    assets.icons.forEach(function(a){var b=$.CreatePanel("Button",p("MainHudTop"),"MainHudEntry_"+a.id);b.AddClass("MainHudIconButton");b.hittestchildren=false;var icon=image(b,a.id+"_64.png");
        // Account for transparent padding: a 64px canvas is not a 64px glyph.
        var s=a.target_glyph_px[0]/a.paint_bounds[2],size=64*s;
        place(icon,(57-size)/2,(28-a.paint_bounds[3]*s)/2-a.paint_bounds[1]*s+5,size,size);
        label(b,"",a.label);b.SetPanelEvent("onactivate",function(){activate(a.id);});tip(b,function(){return available(a.id)?a.label:a.label+"：功能待接入";});topButtons[a.id]=b;});
    ...
    function mirror(){[["MainHudHeroName","SurvivalHeroName"],["MainHudHeroLevel","SurvivalHeroLevel"],["MainHudHealthText","SurvivalHeroHealthText"],["MainHudManaText","SurvivalHeroManaText"]].forEach(function(pair){var source=p(pair[1]);if(source)text(pair[0],(pair[0]==="MainHudHeroLevel"?"Lv.":"")+source.text);});
        ...
        Object.keys(topButtons).forEach(function(id){topButtons[id].SetHasClass("Unavailable",!available(id));});
    }
