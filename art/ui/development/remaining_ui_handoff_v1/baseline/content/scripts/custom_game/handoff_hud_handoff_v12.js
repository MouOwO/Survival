(function () {
    ...
    if(Game.AddCommand){
        ...
        Game.AddCommand("handoff_select"+suffix,function(){var args=Array.prototype.slice.call(arguments);$.Msg("[HANDOFF_SELECT_ARGS] ",JSON.stringify(args));var unit=Number(args[args.length-1]);if(unit>=0&&Entities.IsValidEntity(unit)&&Entities.GetUnitName(unit)){GameUI.SelectUnit(unit,false);GameUI.SetCameraTargetPosition(Entities.GetAbsOrigin(unit),0);$.Msg("[HANDOFF_SELECT] ",unit," ",Entities.GetUnitName(unit)," origin=",JSON.stringify(Entities.GetAbsOrigin(unit)));}},"Select an existing test entity",0);
