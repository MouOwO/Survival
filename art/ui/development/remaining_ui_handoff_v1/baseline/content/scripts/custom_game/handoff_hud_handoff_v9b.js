(function () {
    ...
    if(Game.AddCommand&&!cfg.HandoffCommandsV10){
        cfg.HandoffCommandsV10=true;
        Game.AddCommand("handoff_dump_v10",function(){if(cfg.HandoffDump)cfg.HandoffDump();},"Read-only inspection of isolated handoff HUD",0);
        Game.AddCommand("handoff_units_v10",function(){var rows=CustomNetTables.GetAllTableValues("survival_ability_runtime")||[];rows.forEach(function(row){if(/^unit:/.test(row.key))$.Msg("[HANDOFF_UNIT] ",row.key," ",JSON.stringify(row.value));});},"Inspect test runtime unit identities",0);
        Game.AddCommand("handoff_select_v10",function(id){var unit=Number(id);if(unit>=0&&Entities.GetUnitName(unit)){GameUI.SelectUnit(unit,false);GameUI.SetCameraTarget(unit);$.Schedule(.5,function(){GameUI.SetCameraTarget(-1);});}},"Select an existing test entity",0);
        Game.AddCommand("handoff_combat_debug_v4",function(){cfg.HandoffShowCombatDebug=!cfg.HandoffShowCombatDebug;},"Toggle existing combat diagnostic panel in the test HUD",0);
    }
