(function () {
    ...
    $.Msg("[HANDOFF_BUILD] selection_slots_v14 generation=",generation);
    ...
    function layout() {
        ...
        if(signature!==lastSignature||!ready||!valid(natives.abilities)||!valid(natives.inventory)) {
            ...
            ["hp","mp"].forEach(function(type,index){var y=175+index*60;
                ...
                nine(center,"Handoff_"+type+"_frame",type+"_frame",16,y,g.barWidth,52,10,10);
                style(slices["Handoff_"+type+"_frame"],{zIndex:"1"});
    ...
    cfg.SurvivalMainHUD={Inspect:function(){return {nativeReady:ready,presented:presented,missing:missing,geometry:geometry,unit:selectedUnit(),abilities:currentEntries.map(function(e){return e.name;}),name:nodes.HandoffName.text,hp:nodes.Handoff_hp_value.text,keys:skillPanels.map(function(p,i){var b=nodes["HandoffKey_"+i+"Bounds"];return b&&b.visible?nodes["HandoffKey_"+i].text:"";}),version:"selection_slots_v14",sequence:sequence};}};
