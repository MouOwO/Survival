(function () {
    ...
    $.Msg("[HANDOFF_BUILD] selection_slots_v9 generation=",generation);
    ...
    function mirrorKeys(){
        ...
        for(var i=0;i<32;i++){
            ...
            var matching=binding&&binding.unit===unit&&currentEntries.some(function(e){return Number(e.ability)===binding.ability;});
    ...
    function layout() {
        ...
        if(signature!==lastSignature||!ready||!valid(natives.abilities)||!valid(natives.inventory)) {
            ...
            $.Msg("[HANDOFF_ABILITIES] ",JSON.stringify(currentEntries));
        }
    }
    ...
    cfg.SurvivalMainHUD={Inspect:function(){return {nativeReady:ready,presented:presented,missing:missing,geometry:geometry,unit:selectedUnit(),abilities:currentEntries.map(function(e){return e.name;}),version:"selection_slots_v9",sequence:sequence};}};
    ...
    if(Game.AddCommand){
        ...
        Game.AddCommand("handoff_select"+suffix,function(){var args=Array.prototype.slice.call(arguments),arg=args[args.length-1];var unit=arg==="hero"?Players.GetPlayerHeroEntityIndex(Game.GetLocalPlayerID()):Number(arg);if(unit>=0&&Entities.IsValidEntity(unit)&&Entities.GetUnitName(unit)){GameUI.SelectUnit(unit,false);GameUI.SetCameraTargetPosition(Entities.GetAbsOrigin(unit),0);$.Msg("[HANDOFF_SELECT] ",unit," ",Entities.GetUnitName(unit)," origin=",JSON.stringify(Entities.GetAbsOrigin(unit)));}},"Select an existing test entity",0);
