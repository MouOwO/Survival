(function () {
    ...
    $.Msg("[HANDOFF_BUILD] clean_edges_v17 generation=",generation);
    ...
    function art(parent,id,key) {var p=create("Image",parent,id,false);p.SetImage("file://{images}/"+assets[key].file);return p;}
    ...
    var nav=[["return","返回"],["settings","设置"],["treasure","宝物"],["archive","存档"],["equipment","装备"],["lottery","抽奖"],["benefit","福利"],["appearance","外观"],["social","社交"]];
    ...
    place(art(top,"","top_wave"),734,21,32,32);place(label(top,"HandoffWave"),779,14,218,38);
    [["gold",1193,1233],["wood",1350,1390],["population",1484,1524]].forEach(function(a){place(art(top,"","top_"+a[0]),a[1],20,32,32);place(label(top,"HandoffResource_"+a[0]),a[2],14,a[0]==="population"?143:112,38);});
    ...
    function fitNativeSkills(){
        ...
        candidates.forEach(function(p,index){if(index>=currentEntries.length){style(p,{visibility:"collapse",width:"0px",marginRight:"0px"});p.__handoffOverflow=true;}else{square(p);style(p,{marginRight:"4px"});}});
    }
    ...
    function layout() {
        ...
        if(signature!==lastSignature||!ready||!valid(natives.abilities)||!valid(natives.inventory)) {
            ...
            ["hp","mp"].forEach(function(type,index){var y=175+index*60;
                ...
                // Draw the approved transparent frame as one quad. Separate nine-
                // slice images introduced independently sampled joins when scaled.
                var frameId="Handoff_"+type+"_frame";
                var barFrame=nodes[frameId]||art(center,frameId,type+"_frame");
                place(barFrame,16,y,g.barWidth,52);style(barFrame,{zIndex:"1"});
    ...
    ["dota_player_update_selected_unit","dota_player_update_query_unit","dota_ability_changed"].forEach(function(event){GameEvents.Subscribe(event,function(payload){
        ...
        var pid=payload&&(payload.PlayerID!==undefined?payload.PlayerID:payload.player_id);if(pid!==undefined&&Number(pid)!==Number(Game.GetLocalPlayerID()))return;
        var serial=++eventSerial;lastSignature="";
        if(cfg.HandoffCombat)cfg.HandoffCombat.RefreshSelection();refreshNow();
        [0,.016,.05].forEach(function(delay){$.Schedule(delay,function(){if(serial!==eventSerial||cfg.HandoffGeneration!==generation)return;if(cfg.HandoffCombat)cfg.HandoffCombat.RefreshSelection();refreshNow();});});
    });});
    GameEvents.Subscribe("survival_ui_private_snapshot",data);
    cfg.SurvivalMainHUD={Inspect:function(){return {nativeReady:ready,presented:presented,missing:missing,geometry:geometry,unit:selectedUnit(),abilities:currentEntries.map(function(e){return e.name;}),name:nodes.HandoffName.text,hp:nodes.Handoff_hp_value.text,keys:skillPanels.map(function(p,i){var b=nodes["HandoffKey_"+i+"Bounds"];return b&&b.visible?nodes["HandoffKey_"+i].text:"";}),version:"clean_edges_v17",sequence:sequence};}};
    cfg.HandoffDump=function(){
        $.Msg("[HANDOFF_INSPECT] ",JSON.stringify(cfg.SurvivalMainHUD.Inspect()));
        for(var ancestor=native("InnateIcon"),depth=0;valid(ancestor)&&depth<4;ancestor=ancestor.GetParent(),depth++){var children=[];for(var c=0;c<ancestor.GetChildCount();c++){var cp=ancestor.GetChild(c);children.push({id:cp.id,type:cp.paneltype,w:cp.actuallayoutwidth,h:cp.actuallayoutheight,bg:cp.style.backgroundImage});}$.Msg("[HANDOFF_REMAINDER] ",ancestor.id," bg=",ancestor.style.backgroundImage," children=",JSON.stringify(children));}
        function dump(p,depth){if(!valid(p)||depth>8)return;$.Msg("[HANDOFF_NODE] ",p.id," type=",p.paneltype," parent=",p.GetParent()?p.GetParent().id:"", " size=",p.actuallayoutwidth,"x",p.actuallayoutheight," pos=",p.GetPositionWithinWindow?JSON.stringify(p.GetPositionWithinWindow()):"", " visible=",p.visible," opacity=",p.style.opacity," transform=",p.style.transform);for(var i=0;i<p.GetChildCount();i++)dump(p.GetChild(i),depth+1);}
        dump(native("abilities").GetChild(0),0);
        function shadows(p){if(!valid(p))return;if(/shadow|background|backpack|^inventorybg$/i.test(p.id))$.Msg("[HANDOFF_SHADOW] ",p.id," bg=",p.style.backgroundImage," color=",p.style.backgroundColor," shadow=",p.style.boxShadow," opacity=",p.style.opacity);for(var i=0;i<p.GetChildCount();i++)shadows(p.GetChild(i));}
        shadows(native("inventory"));
        function extras(p){if(!valid(p))return;if(/innate|facet|statbranch|composition|neutral/i.test(p.id))$.Msg("[HANDOFF_EXTRA] ",p.id," size=",p.actuallayoutwidth,"x",p.actuallayoutheight," pos=",p.GetPositionWithinWindow?JSON.stringify(p.GetPositionWithinWindow()):"");for(var i=0;i<p.GetChildCount();i++)extras(p.GetChild(i));}extras(native("lower_hud"));
        var pk=native("abilities").FindChildTraverse("SurvivalAbilityHotkey");
        if(pk)$.Msg("[HANDOFF_KEY] text=",pk.text," width=",pk.style.width," height=",pk.style.height," pos=",pk.style.position);
        ["HandoffName","HandoffLevel","Handoff_hp_value","Handoff_mp_value"].forEach(function(id){var p=nodes[id];$.Msg("[HANDOFF_LABEL] ",id," text=",p.text," align=",p.style.textAlign," size=",p.actuallayoutwidth,"x",p.actuallayoutheight);});
    };
    if(Game.AddCommand){
        var suffix="_v17_"+generation;
        Game.AddCommand("handoff_dump"+suffix,function(){cfg.HandoffDump();},"Read-only inspection of isolated handoff HUD",0);
        Game.AddCommand("handoff_units"+suffix,function(){var rows=CustomNetTables.GetAllTableValues("survival_ability_runtime")||{};Object.keys(rows).forEach(function(key){var row=rows[key];if(/^unit:/.test(row.key))$.Msg("[HANDOFF_UNIT] ",row.key," ",JSON.stringify(row.value));});},"Inspect test runtime unit identities",0);
        Game.AddCommand("handoff_select"+suffix,function(){var args=Array.prototype.slice.call(arguments),arg=args[args.length-1];var unit=arg==="hero"?Players.GetPlayerHeroEntityIndex(Game.GetLocalPlayerID()):Number(arg);if(unit>=0&&Entities.IsValidEntity(unit)&&Entities.GetUnitName(unit)){GameUI.SelectUnit(unit,false);$.Msg("[HANDOFF_SELECT] ",unit," ",Entities.GetUnitName(unit));}},"Select an existing test entity without moving camera",0);
        Game.AddCommand("handoff_switchcheck"+suffix,function(){
            var units=Array.prototype.slice.call(arguments,1).map(Number);
            if(units.length<2||units.some(function(u){return !isFinite(u)||!Entities.IsValidEntity(u)||!Entities.GetUnitName(u);}))return;
            var index=0;
            function step(){if(cfg.HandoffGeneration!==generation||index>=18)return;var unit=units[index++%units.length];GameUI.SelectUnit(unit,false);
                $.Schedule(.12,function(){if(cfg.HandoffGeneration===generation)$.Msg("[HANDOFF_SWITCH] expected=",unit," ",JSON.stringify(cfg.SurvivalMainHUD.Inspect()));});$.Schedule(.18,step);}
            step();
        },"Test rapid selection of existing entities; no game orders",0);
        Game.AddCommand("handoff_combat_debug"+suffix,function(){cfg.HandoffShowCombatDebug=!cfg.HandoffShowCombatDebug;},"Toggle existing combat diagnostic panel in the test HUD",0);
    }
