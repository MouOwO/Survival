...
local function apply_cheer_buff(unit, player_id, bonus_pct)
    ...
    if bonus_pct > 0 then
        ...
        if modifier then
            modifier:SetStackCount(math.floor(bonus_pct + 0.5))
...
local function apply_lumberjack_attack(state, technology_attack)
    ...
    local maximum = (base_max + total_growth) * attack_pct
    state.unit:SetBaseDamageMin(minimum)
    state.unit:SetBaseDamageMax(maximum)
    if state.unit.CalculateStatBonus then state.unit:CalculateStatBonus(true) end
    state.unit.survival_attack_min = minimum
...
function M.commit_lumberjack_fusion(materials, target, data)
    ...
    local removed_entindexes = {}
    for _, state in ipairs(states) do
        if state.unit:entindex() ~= target_entindex then
            removed_entindexes[#removed_entindexes + 1] = state.unit:entindex()
    ...
    event_bus.emit(events.UNIT_COMBAT_STATS_CHANGED, {
        ...
        reason = "lumberjack_fusion_completed",
    })
    event_bus.emit(events.WORKER_CHANGED, {
...
local function on_tree_spawned(payload)
    ...
    local previous = player_tree(player_id)
    local next_entindex = payload.entindex or -1
    local next_efficiency = math.max(0, tonumber(payload.lumber_efficiency_buff) or 0)
    local target_changed = previous.entindex ~= next_entindex
    local efficiency_changed = previous.lumber_efficiency_buff ~= next_efficiency
    trees_by_player[player_id] = {
        entindex = next_entindex,
        lumber_efficiency_buff = next_efficiency,
    }
    -- Keep per-player tree isolation and skip unchanged level-cap resets.
    if target_changed then update_worker_targets(player_id) end
    if target_changed or efficiency_changed then update_worker_efficiency(player_id) end
end
